...
- `lazy-experts.researcher` — takes a research asset's approved research design (`design.md`) and writes the research report `research.md`: routes walked, sourced findings, compared options, a conclusion that answers the question and rules on the hypothesis, and the sources. Walks the spec tree, code and wiki first, the internet only after. Also validates a research design in review — the question, never the answer.
...
Offline copy at `~/.claude/plugins/cache/.../plugins/claude/lazycortex-experts/help/`.
<!-- help-block:end -->
