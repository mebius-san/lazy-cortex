...
- **Invented terminology.** A noun phrase coined for this document as if it were an established term — a name-shaped label the reader cannot look up anywhere. A concept that genuinely needs a name goes into the repository's terms dictionary and is introduced per Terminology discipline; everything else is described in plain words, even when the plain description is longer. The test: would a reader searching the repository for this phrase find a definition? No definition anywhere — no coined phrase in the document. **The qualified form of a term the upstream document or the dictionary already carries is not invented terminology** — it is that term written as a name, per Terminology discipline; the ban is on labels nobody can look up, not on the qualifier that tells a term from an everyday word.
...
- **An empty section beats a padded one.** When there is nothing checkable to put under a heading, the section stays empty: a mandatory section keeps its heading and template comment and reads as empty, and a section whose template comment opens with `Optional.` goes out whole, heading and comment together. A restatement of the heading, a generality that holds for any document, or a guess dressed as a fact, written so that the section looks filled, is a defect the review sends back — the reader gets less from it than from the empty heading.
...
- **A term is a name, not a common word.** When the bare word has an everyday meaning or a second technical meaning in the document's language, the term carries a qualifying noun that says which thing of this project it names: *input field*, not *field*; *artifact version*, not *version*; *family member's edit*, not *edit*. The qualifier is part of the term and is inherited with it — a downstream document never strips it back to the bare word. The test: could a reader take this word for its everyday meaning in this sentence? If yes, the word is not yet a term.
- **The `## Terms` section is the document's glossary.** When the template carries a `## Terms` section, every term the document uses stands there as one line, `**term** — definition`, in alphabetical order: a term the repository's dictionary already defines carries the dictionary's definition verbatim, and a term this document introduces is defined there before the body uses it. The section never lists a word a general reader already knows, and is the first thing the dictionary's curator reads when it takes new terms from the finished document. The section is optional in every template that carries it: a document that uses no term worth a definition drops the section whole rather than filling it with everyday words.
- **The dictionary is inherited, not invented.** Names of entities come verbatim from the upstream document the job carries (brief for a spec, spec for a plan, plan for a journal). If the upstream calls it `job dir`, the output says `job dir` — not "task folder". Inheritance covers the names, never the upstream's own prose: a colloquial or broken phrasing found there is normalized in your text, not reproduced. **One exception: the repository's terms dictionary outranks the upstream document.** When the scope has a dictionary and it names the concept differently, take the dictionary's term — upstream is one chain of documents, the dictionary is the whole repository, and following the chain is exactly how two chains end up with two names for one thing. Nothing about the disagreement goes into your document: the upstream document's word is the dictionary curator's finding on the report side, and a reader of your document has no use for it. **A bare word upstream is inherited as a qualified term.** When the upstream document names a project entity with a bare common word, introduce the qualified form once, with its definition sentence, and say nothing about the upstream's bare form — the only case in which an inherited name changes in your text.
- **New terms are introduced once, explicitly.** A concept with no upstream name gets one definition sentence at first use; after that the term is fixed. A second name for an already-named concept is never introduced; the qualified form of an inherited or dictionary term is not a second name but the same term written as a name.
...
- Write every term as a name: a qualifying noun whenever the bare word has an everyday or a second meaning in the document's language; inherit the qualifier with the term, and introduce the qualified form once when the upstream document uses the bare word.
- When the repository's terms dictionary names the concept differently from the upstream document, use the dictionary's term and say nothing about the disagreement in your document.
- Fill the `## Terms` section when the document uses a term worth defining: one alphabetical line per term, the dictionary's definition verbatim for a dictionary term, your own definition for a term the document introduces; drop the section whole when there is no such term, and never pad it with everyday words.
- Leave a section empty when you have nothing checkable to put under it — a mandatory section keeps its heading and template comment, a section whose comment opens with `Optional.` goes out whole — and never write prose whose only job is to make the section look filled.
