...
description: "Cross-cutting active-research discipline composed onto every lazy-experts specialist. Requires the expert to actively gather knowledge before writing — the system documents of the content root, of every enclosing product, then of the product itself (vision, use-cases, design, ui-design, tech), whichever exist, then research-marked pull-skills (structure maps, domain trees, term dictionary once shipped) discovered dynamically rather than named, a covered wiki scope, and greps of the surrounding tree — and to surface a gap as a question rather than invent an answer, independent of the expert's role or domain."
---
...
| Question | Action |
|---|---|
| What do the levels above this doc already say about themselves? | `Read` the system documents of the content root first — `vision.md`, `use-cases.md`, `design.md`, `tech.md` — then those of every product enclosing this doc's product, outermost first, then the owning product's own: `vision.md`, `use-cases.md`, `design.md`, `ui-design.md`, `tech.md` at each product's spec root (the product chain comes from `spec_path` prefixes in `lazy.settings.json[products]`), FIRST, before any other research step. This is the mandatory opening read, not an optional one. Read each one that EXISTS; an absent one is a determination, never a gap to report. No link points at these documents from the doc being written — a level document is read by rule, not by reference, and `spec_source_docs` deliberately does not carry them. |
...
- Read the content root's own four (no `ui-design.md` there), then every enclosing product's own `vision.md` / `use-cases.md` / `design.md` / `ui-design.md` / `tech.md` outermost first, then the owning product's own, before writing anything, every time — the first mandatory step, not a fallback. Whichever exist; the rest are determinations, not gaps.
