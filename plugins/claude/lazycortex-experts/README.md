...
- **execution-stage-agents** — Nine agents that carry approved work into deliverables. `lazy-experts.implementer` follows a plan test-first, `lazy-experts.data-implementer` writes data files straight from a content design, `lazy-experts.docs-writer` writes user-facing documentation from the design, `lazy-experts.debugger` explains a failure before fixing it, `lazy-experts.reviewer` judges a change without editing it, `lazy-experts.tester` exercises what actually runs, `lazy-experts.fiction-writer` produces literary prose from an outline, and the two editors correct prose someone else has already written without touching what it says — `lazy-experts.editor` on technical documents, `lazy-experts.fiction-editor` on literary text. Members: lazy-experts.implementer, lazy-experts.data-implementer, lazy-experts.docs-writer, lazy-experts.debugger, lazy-experts.reviewer, lazy-experts.tester, lazy-experts.editor, lazy-experts.fiction-writer, lazy-experts.fiction-editor.
...
- [troubleshooting](https://github.com/mebius-san/lazy-cortex/blob/main/plugins/claude/lazycortex-experts/help/troubleshooting.md) — Common failure modes during lazycortex-experts setup — symptoms, likely causes, and fixes.
- [faq](https://github.com/mebius-san/lazy-cortex/blob/main/plugins/claude/lazycortex-experts/help/faq.md) — Common questions about installing lazycortex-experts, the class map, composing specialists, auditing the composition, and the sixteen generic agents' lane boundaries.
...
| `lazy-experts.researcher` | Use when a research asset's approved research design (`design.md` typed `research-design`) states the questions and the research report `research.md` still needs writing — the routes walked, findings each with a source, compared options, a conclusion that answers every question and rules on each hypothesis, and the sources — from the spec tree, the code, the wiki, and the internet once the internal routes run dry. Dispatched by the expert runtime for any `researcher`-class expert as the `research` tool's job; also the validator of a research design in review. Pick it over the interpreter when the questions are already settled and only the answer is missing, and over the designer when nothing is being built — the deliverable is the report, and an approved report is immutable. |
