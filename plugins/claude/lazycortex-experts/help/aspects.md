...
diagram_spec:
  - anchor: "Domain aspects feed the composition entry"
    ...
    kind_hint: flow
  - anchor: "Cross-cutting aspects and the two outcomes"
    request: "Flow diagram: discipline-aspect and research-aspect feed a single lazy.settings.json[experts] composition node labelled 'every entry'; tech-writing-aspect, terms-aspect, and structure-aspect feed the same composition node labelled 'technical entries only'. The composition node then feeds two outcome nodes, Technical specialist and Fiction specialist, labelled 'technical class' and 'fiction class' respectively. No other nodes."
    kind_hint: flow
source_skills:
...
**`lazy-experts.discipline-aspect`** is a cross-cutting aspect that composes onto every seeded specialist regardless of class — technical or fiction. It adds five iron laws the expert holds itself to regardless of its role or domain: verify before claiming done (evidence must be named in the document, never implied, and re-checked pass after pass until one comes back clean — capped at five passes, a repeated finding, or a pass that breaks more than it fixes), never guess past an input gap (surface the gap and stop rather than inventing an answer and proceeding), no performative agreement (evaluate the operator's input technically; push back with reasons if it is wrong), read decisions before starting work (declare only a genuine fork explicitly, with its reason and the alternatives honestly rejected, and never quietly revisit one already made), and treat operator-deleted content as a one-way door (content the operator removed or a proposal it rejected never comes back in any wording unless an explicit operator instruction restores it or the expert's own `[!question]` makes the case and gets answered first). It also adds the async-translation principle: wherever a synchronous workflow would pause to ask a human, the expert translates that gate into an open point in the document instead, then stops and waits for the operator to answer there. Because it is role-independent, it carries no domain discovery and adds no tooling of its own; the shape of open-point callouts, checkboxes, and markers is always defined by the dispatching protocol, not by this aspect.

**`lazy-experts.research-aspect`** is the second universal cross-cutting aspect — it composes onto every seeded specialist alongside discipline, the active-research counterpart to a dispatch's own `source` / `context` bundle. It requires the expert to look beyond what it was handed before writing: read the content root's own `vision.md`, `use-cases.md`, `design.md` and `tech.md` first, then `vision.md`, `use-cases.md`, `design.md`, `ui-design.md` and `tech.md` at the spec root of every product enclosing this doc's product, outermost first, and last the owning product's own — always, as the mandatory opening step; whichever of them exist, an absent one is a determination rather than a gap to report; then walk sibling and ancestor docs in the surrounding spec tree; then discover whichever research-marked pull-skills exist in the current install (structure maps, domain trees, a term dictionary) by spotting the word "research" in a skill's description rather than by a hardcoded list; then query a covered wiki scope; and only then grep the code itself. An absent research route — a pull-skill not installed, no structure map built, no wiki scope covering the question — is treated as a configuration fact, not a knowledge gap: the expert degrades silently to reading the code and never recommends installing or configuring the missing tooling in its output. Only a gap that survives every route becomes a surfaced question, through the expert-signals channel the dispatching protocol defines, following the same async-translation principle discipline already applies.

**`lazy-experts.tech-writing-aspect`** is a cross-cutting aspect that composes onto technical-class specialists only — fiction classes (`sci-fi`, `fantasy`) never carry it, because its bans contradict literary craft. It governs the sentences a technical document is made of, not what the specialist produces: no metaphor or figurative imagery, no atmospheric openings, no evaluative epithets ("elegant", "robust") without a measurable property behind them, no emotional intensifiers, no shop talk or informal register, no invented terminology (a coined noun phrase presented as an established term with no definition a reader could look up — though the qualified form of a term the upstream document or the dictionary already carries is not invented terminology; it is that term written as a name, not a fresh label nobody can look up), no invented abbreviations, no nominalized clause chains (a claim states one subject and one concrete verb, using the document's own names for both — swapping a named operation for an abstract nominal turn reads shorter but stops parsing, and a sentence that takes more than two clauses to parse gets split instead), and no filler sentence that carries no checkable fact or obligation. These bans, and the rest of the aspect's sentence-construction rules, carry the specification-prose canon verbatim — ISO/IEC/IEEE 29148, IEEE 830, the INCOSE Guide for Writing Requirements, and ASD-STE100 — down to a structural rule: any enumeration of three or more items, or any items that carry their own detail, is written as a markdown list with one item per line, never strung through a sentence with semicolons, commas, or dashes, and an empty section beats a padded one — when there is nothing checkable to put under a heading, a mandatory section keeps its heading and stays empty rather than being written around, and a section whose template comment opens with `Optional.` goes out whole, heading and comment together, instead of being filled with a restatement or a guess dressed as fact. It also enforces a single-term-per-concept dictionary inherited verbatim from the upstream document the job carries (a spec inherits its terms from the brief, a plan from the spec) — except that when the repository's terms dictionary names the same concept differently, the dictionary's term wins silently: the document carries no note about the upstream's word, because that word is a finding for the dictionary's own curator, not something a reader of the document has any use for. A term is a name, not a common word: whenever the bare word has an everyday meaning or a second technical meaning in the document's language, the specialist writes it with a qualifying noun that says which project entity it names — an input field rather than a field, an artifact version rather than a version — and inherits that qualifier along with the term rather than stripping it back to the bare word later. When the upstream document itself uses the bare form for a concept that needs qualifying, the specialist introduces the qualified form once, with its definition sentence, and says nothing about the upstream's bare form — the one case in which an inherited name changes in the specialist's own text. A new term, qualified or not, gets one definition sentence at first use and stays fixed after that. When the document's own template carries a `## Terms` section, the aspect requires the specialist to fill it with one alphabetical line per term the document uses, `**term** — definition`, the dictionary's wording verbatim for a dictionary term and the specialist's own definition for one it introduces, never padded with everyday words a general reader already knows — and the section is itself optional per the empty-section rule above: a document that uses no term worth defining drops it whole rather than being filled for appearance's sake. That section, when it survives, is the first thing the terms dictionary's curator reads when it later takes new terms from the finished document. Use it on any specialist that writes specs, plans, or reviews — never on `fiction-writer`.

**`lazy-experts.terms-aspect`** is a cross-cutting aspect that composes onto technical-class specialists only, alongside tech-writing. Where tech-writing enforces one term per concept *inside* a document, terms-aspect supplies the term from *outside* it, so documents written months apart by different experts converge on the same word: before naming a concept, the specialist asks the `lazy-wiki.terms` skill whether the repository dictionary already has a term for it, and uses the dictionary's word verbatim even when its own reads better. When the dictionary's heading is a qualified name and the specialist was about to write the bare head word, that is the same concept and the qualified term wins — the bare word is not a shorter synonym, it is the everyday word the qualifier exists to escape. When the dictionary carries a project entity under a bare common word a reader could mistake for its everyday meaning, the specialist writes the qualified form in its own document and lists it in `## Terms` with the dictionary's definition verbatim, naming the bare heading nowhere in its own document — the curator matches the verbatim definition back to the dictionary's own heading and renames it there; that move stays the curator's, never the specialist's. The specialist is forbidden from writing to the dictionary under any circumstance — not to add a term it just coined, not to fix a definition, not to correct a typo, not to rename a heading; the dictionary's curator reads finished documents and updates it later. When the dictionary and the job's upstream document name a concept differently, the dictionary's term wins and the specialist says nothing about the disagreement — the upstream's word is a finding for the curator's report, not for this document's reader. When a document the specialist writes carries a `## Terms` section, every term it used stands there — a dictionary term with the dictionary's definition verbatim, a term of its own with the definition it introduces it by — because that section is where the curator reads new terms from, and a term missing there is a term the dictionary may never learn. When the `lazy-wiki.terms` skill is absent, the repository has no dictionary and the specialist writes as it normally would, saying nothing about the gap.

**`lazy-experts.structure-aspect`** is a cross-cutting aspect that composes onto technical-class specialists only. It routes every "where does this live" or "where does new work belong" claim through the repository's structure map via the `lazy-wiki.structure` skill's `query [<path>]` mode — a targeted slice, never the whole map read into context. The specialist is forbidden from writing to `docs/structure.md`; when the map's entry contradicts what the specialist then finds on disk, it trusts the disk and says nothing about the stale entry in its own document — repairing the map is the curator's and the rebuild's job, never a self-correction made here. When the map offers two plausible homes for new work, the specialist names the choice and its reason instead of picking silently. When the `lazy-wiki.structure` skill is absent, or reports no map configured, the specialist grounds the same placement claim by reading the tree directly and says nothing about the missing tooling.
...
**`lazy-experts.sci-fi-aspect`** adds science-fiction genre expertise to whichever expert composes it — in practice, `fiction-writer` or `fiction-editor`. It treats the story's speculative premise as a system with consequences rather than a backdrop: name the novum (the one invented difference the story runs on) and work its second-order effects through plot, society, and scene; keep extrapolation coherent with whatever the story has already stated about its own technology; let the technology's limits — cost, latency, scarcity, failure — create the dramatic pressure instead of dissolving it. It is neutral on subgenre (hard SF, space opera, cyberpunk, near-future) and matches its plausibility rigor to whichever pin the brief sets. It also requires native-grade, calque-free prose in whatever language the piece is written — verbs collocating with their objects by that language's own norms, grammar and idiom held throughout narration, with a deliberate stylization (an archaic register, a broken voice) staying attributable to one character rather than reading as the author's own error.

**`lazy-experts.fantasy-aspect`** adds fantasy genre expertise to whichever expert composes it — in practice, `fiction-writer` or `fiction-editor`. It treats the invented world as a constraint on every scene: magic shows or implies a cost every time it's used, world details that don't change a character's choices or stakes are cut, and established rules, geography, history, and names bind every later sentence — a contradiction with earlier text or the story bible is a defect, not a creative choice. Names and languages stay coherent with their culture's established conventions, and wonder is anchored in what a marvel does to characters and stakes rather than in unattached adjectives. It is neutral on subgenre (epic, urban, dark, fairy-tale) and, like the sci-fi aspect, requires native-grade, calque-free prose — grammar, collocation, and idiom held to the language's own norms throughout narration, with any deliberate stylization staying attributable to a single voice rather than passing as an author's error.
...
- The **agents** block (`plugins/claude/lazycortex-experts/help/agents.md`) describes the generic agents that aspects compose onto — the engineering roles plus `fiction-writer`, the one that pairs with `sci-fi-aspect` and `fantasy-aspect`.
...
Each domain aspect is a file the composition entry names; the six technical ones pair with the engineering agents, the two genre ones pair with `fiction-writer` and `fiction-editor`.
