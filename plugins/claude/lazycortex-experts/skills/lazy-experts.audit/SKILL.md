...
- **Plugin root** — the `installPath` field for `lazycortex-experts@lazycortex` in `~/.claude/plugins/installed_plugins.json`. When the repo at hand authors this plugin, `plugins/claude/lazycortex-experts/` is the root instead and wins, so the audit judges the sources being edited rather than a stale cached copy. Neither present → `FAIL plugin-root-unresolved — fix: /plugin install lazycortex/lazycortex-experts`; stop, no further phase runs.
...
Read the `experts` section with `Bash("${LAZYCORTEX_PYTHON:-python3}" <core-cli> settings-get experts --cwd <repo-root>)`, which reads the tracked file only, so a local overlay is never mistaken for tracked config.
