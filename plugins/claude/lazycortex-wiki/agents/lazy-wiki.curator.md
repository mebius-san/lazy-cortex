...
```bash
find $(echo "$LAZYCORTEX_PLUGIN_DIRS" | tr ':' ' ') -maxdepth 2 -path '*/bin/lazycortex-wiki' 2>/dev/null | head -1
```
...
4. **Apply (ALWAYS, both modes).** `"${LAZYCORTEX_PYTHON:-python3}" "$WIKI_BIN" apply-node <abs-node-path> --from <curation-file>` — MUST exit 0. On failure: tail:true → write `result/response.json` `outcome=error` and stop; tail:false → report the error and stop. In tail:false, `rm` the temp file after a successful apply.
5. **(tail: true only)** `"${LAZYCORTEX_PYTHON:-python3}" "$WIKI_BIN" build-index <scope-id> --repo <repo-root>` → `git add <abs-node-path> <abs-topics-md-path> && git commit -m "wiki(classify): <node-basename>"` — do **NOT** pass `--author`; the pump set `GIT_AUTHOR_NAME` / `GIT_AUTHOR_EMAIL` in env, git picks them up automatically. Never widen this to a wildcard: the index is shared. → `"${LAZYCORTEX_PYTHON:-python3}" "$WIKI_BIN" dispatch-link <abs-node-path> --repo <repo-root>`.
...
4. **Apply (ALWAYS, both modes).** `"${LAZYCORTEX_PYTHON:-python3}" "$WIKI_BIN" apply-node <abs-node-path> --from <curation-file>` — MUST exit 0. In tail:false, `rm` the temp after a successful apply.
5. **(tail: true only)** `git add <abs-node-path> && git commit -m "wiki(link): <node-basename>"` — do **NOT** pass `--author`; the pump set `GIT_AUTHOR_NAME` / `GIT_AUTHOR_EMAIL` in env, git picks them up automatically. Never widen this to a wildcard: the index is shared.
