...
diagram_spec:
  ...
  - anchor: "Audit"
    request: "Flow diagram of lazy-wiki.audit, which is read-only. It audits the wiki scopes via the CLI, then reports on the terms scopes and the structure map by reading plus a curator dispatch in report mode; it labels fixable wiki findings and names `"${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> doctor <scope-id> --apply` — run by hand, or applied daily by the scheduled lazy-wiki.doctor-apply routine — as their repair route. The skill itself writes nothing. /lazy-wiki.help is optional orientation at any point."
source_skills:
  - lazy-wiki.install
  - lazy-wiki.configure
  - lazy-wiki.audit
...
surface_sha: 95264d61cb30027a798ff14911f243cc7caf3590212d796cee9551aa6dcdf71f
---
...
- **Structure branch** (`/lazy-wiki.configure structure`) — sets `depth_profiles` (named classes of globs, each with a depth of `file`, `dir`, or `brief`) and the `exclude` list for the fixed-path project-structure map, `docs/structure.md` (which is always kept in its own exclude list, alongside `.memory/**`, so the map never describes itself and an expert's own memory-note commit never wakes the scan). It registers three git-watch routines — for new, deleted, and renamed files — since `renamed_files` exposes different placeholders than the other two and needs its own request template; the map describes the tree's shape, not file contents, so the scan deliberately does not watch plain content edits — only a file appearing, disappearing, or moving triggers a dispatch. Every changed path the tick observes reaches the curator as one job — the routines carry no grouping key of their own, since the curator's batch request already expects every changed path in one list, and a routine still carrying a per-file or per-directory grouping key from before this shape is drift `/lazy-wiki.audit` flags and this branch's re-registration clears. Once the routines are registered, the branch also builds the initial `docs/structure.md` itself when it's missing (via `/lazy-wiki.structure rebuild`), rather than leaving that as a step for you to remember — the routines only ever keep an existing map current, they never create one.
- **Vault branch** (`/lazy-wiki.configure vault`) — edits the three repository-wide keys directly rather than through any one scope: `tag_axes`, the closed axis vocabulary the whole vault classifies against; `exclude`, the glob list unioned into every scope's `exclude_paths`; and `language`, the optional ISO 639-1 code the wiki's own writers (the curator's summaries and glosses) write in. `doc-kind` and `docs/structure.md` stay put no matter what you type — dropping either here is undone by the next `/lazy-wiki.install` run — and removing any other axis is flagged before it's written, since every node already tagged on that axis goes unknown and every scope narrowed to it silently loses it. The language question is read-first and asked at most once: leave it on `inherit` and the wiki follows the repository-wide top-level `language` (`en` if that's unset too), or pick `set-own` to record a code just for wiki writers; a code already on record is never re-asked. It's a separate setting from the Domains branch's own `wiki.domains.language`, which only governs the generated domain-spec docs.
...
`/lazy-wiki.audit` audits a single scope or every configured wiki scope, and — in the same run — the terms dictionaries and the structure map, if either is configured. It is read-only from start to finish: it never applies a repair itself, and it writes nothing. For wiki scopes it surfaces findings at four severities (`PASS`, `INFO`, `WARN`, `FAIL` — a check that ran with nothing wrong is a `PASS`) and labels which are fixable: orphan topics, index desync, and a stale index after a node moves or a tag lands on an untracked node (`topics.md` links that point at nothing, or a tagged node the index doesn't yet mention) — all repaired by rebuilding the topic index, non-canonical See-also link paths (rewritten to the canonical target), broken See-also lines (dropped), and stale glosses (refreshed) — each fixable finding names `Bash("${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> doctor <scope-id> --apply)` as its repair route, the same command the scheduled `lazy-wiki.doctor-apply` routine runs daily (with `--commit`) on your behalf, so you can either run it by hand or let the routine catch it. A third wiki-scope check stands apart from the fixable/report-only split: a scope whose `exclude_paths` swallow everything its `paths` admit is flagged `WARN scope-self-nullified` — the scope indexes nothing, its declared `topics_index` never gets built, and every session the navigation rule sends to that index finds it empty; the finding says that removing the offending glob would wake a curator job for every file the scope then covers, so lifting the exclusion is a deliberate choice, not a typo fix — it gives no file count, a finding states what is, not what a repair nobody has run would cost. Everything else — including the domain-generation checks (a stale hash, an unknown group, a missing gloss, a scope glob reaching the structurally-excluded output tree, a generated domain doc carrying a `wiki/<axis>/…` tag whose axis isn't in `wiki.tag_axes`) and the mirror checks (an orphaned clone, local edits a sync would overwrite, `paths` not covering `mirror_path`) — is report-only, with its own hand-repair route named alongside the finding. For terms scopes and the structure map, the audit pairs two kinds of check: configuration and format problems the skill judges directly by reading (a missing dictionary file, overlapping scope globs, a routine whose filter no longer matches, one of the structure map's three scan routines registered while `docs/structure.md` doesn't exist yet, `docs/structure.md` missing from the repository-wide `wiki.exclude` while some scope's `paths` still cover it) — plus the one hard `FAIL` in this section, `structure-watch-filter`: it runs the CLI's `structure-watch-config --check` rather than judging by reading, comparing what the three structure-scan routines actually watch against what the map's `depth_profiles` and `exclude` say they should — and flagging any routine still carrying a retired per-file or per-directory grouping key — and reports a mismatch per routine. This one is a `FAIL`, not a `WARN`, because the cost is unbounded: a routine with no filtering dispatches jobs for trees the map doesn't describe at all, and a routine still grouped per file or per directory splits one tick's changes into many jobs instead of the single job carrying every changed path the curator's batch request expects — either way a single large refactor commit can burn a day's job budget on jobs that all answer `noop`; the repair is re-running `/lazy-wiki.configure structure`, which re-registers the drifted routines. The audit's second kind of structure/terms check hands meaning problems to the owning curator in report mode — `wiki.terms-curator` returns divergence / missing / duplicate / dead terms, `wiki.structure-curator` returns entries the map and the tree disagree on. Neither curator writes anything in this mode. Every finding the audit prints carries its own repair route; terms and structure findings are never batch-applied — each is decided with you one at a time, since which word is "right" is a judgment call, not a default, and a document under active review or inside an upstream mirror is never touched either way.
...
**Setting or changing the language the wiki's own writers write in** — run `/lazy-wiki.configure vault`; it asks once whether the wiki should inherit the repository-wide `language` (the default) or record its own ISO 639-1 code, and never re-asks once a value is on record. This is separate from the generated domain docs' language, which lives under `/lazy-wiki.configure domains` instead.
...
**Reshaping the structure map's classes or depths, or fixing a routine watching the wrong paths (`/lazy-wiki.audit`'s `structure-watch-filter` FAIL)** — run `/lazy-wiki.configure structure`; existing classes are offered for keep/change/remove before you're asked about new ones, and every run re-registers the three scan routines against the current `depth_profiles` and `exclude`, healing any drift the audit flagged.

**Waking up a scope the audit flagged `scope-self-nullified`** — run `/lazy-wiki.configure` against that scope id and narrow or drop the `exclude_paths` glob the finding named; removing it wakes a curator job for every file the scope then covers, so treat it as a deliberate choice rather than a typo fix, not something to do by reflex.
...
flowchart LR
  ...
  seedStructureExclude["Seed structure exclude: docs/structure.md and .memory/**"]
  ...
  seedTagAxesAndExclude -->|seeds structure excludes| seedStructureExclude
  seedStructureExclude -->|unions axis| unionDocKindAxis
  ...
  class seedStructureExclude action
...
flowchart LR
  ...
  runCliAudit["Run "${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> doctor via CLI"]
  ...
  nameRepairRoute["Name "${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> doctor scope-id --apply as repair route"]
