...
**Manual runs** (no daemon needed): mirror sync is a plain CLI call from any session — `"${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> mirror-sync <scope-id>` fetches the source, syncs the mirror, and commits; `"${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> mirror-plan <scope-id>` prints the dry-run plan as JSON. The daemon counterpart is the `lazy-wiki.mirror-sync.<scope-id>` schedule routine registered by `/lazy-wiki.install`. `"${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> terms-apply <document> --from "<word>" --to "<term>"` applies a terminology decision you have already made to one document — whole-word, prose only, never frontmatter, fences, inline code, link targets or the `# See also` block — and refuses on a document under an open review round or inside a mirror tree.
...
Offline copy at `~/.claude/plugins/cache/.../plugins/claude/lazycortex-wiki/help/`.
<!-- help-block:end -->
