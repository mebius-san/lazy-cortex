...
Parse the JSON: `changed_groups` (each entry: `group`, `hash`, `files`, `doc_path`, `gloss`, `blocks`, `contracts`, `tag_axes`, `existing_tags`, `tag_dictionary` — the last three are the tag-canon inputs the writer needs), `orphaned_docs`, `unlisted_docs`, `unknown_groups`, `index_path`, `index_needs_update`, `language`, `dictionary`, `output`. Report any `unknown_groups` to the operator verbatim (groups used in code but absent from the dictionary — `/lazy-wiki.audit` territory; this run skips them, never generates them).
