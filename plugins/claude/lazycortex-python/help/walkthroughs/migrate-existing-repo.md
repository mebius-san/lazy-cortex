...
surface_sha: e5eae66ee8a4195bc58b9077642f728c4a1180a1fe596269f7326bf8919982ae
---
...
- **Cache markers.** `pcf` requires every definition under a `functools` cache decorator (`cache`, `lru_cache`, `cached_property` — bare or qualified, called or not) to carry an `# opt:` clause in the contiguous comment block directly above the decorator stack, naming the assumption the cache rests on — which inputs the stored value depends on, what invalidates it. Pre-existing caches almost never carry this marker, so expect one finding per undocumented cache (`cached '<name>' has no '# opt:' clause naming what the cache assumes`), anchored on the first decorator line. Fix by adding the `# opt:` clause above the decorator, not by removing the cache; a specific case can be exempted with a `# waiver: <reason>` on the decorator line instead. A hand-rolled memo dict or precomputed table has no decorator to key on and isn't caught by this check — it stays a review-phase finding.
