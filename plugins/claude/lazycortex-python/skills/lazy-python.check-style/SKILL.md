...
- **Marker clauses** — every `# opt:` names the assumption outside the marked lines that keeps the optimization correct (a cache, memo, or precomputed table without one is an issue; `pcf` catches only the `functools` decorators); every `# limit:` names the ceiling and the upgrade path past it, and sits on code that is finished as written (unfinished work is `TODO:`); every `# Decision:` records a real fork per the canon's three tests; every `Contract:`, `Domain(…):`, and `opt:` block in changed code has a test derived from it, and every `opt:` in a file whose contracts, domain blocks, or function bodies changed still holds against the change. `pcf` proves the clause is non-empty; judging what it says is this pass. A `Domain(unfiled):` block is a parked marker rather than a violation — route it to `/lazy-python.knowledge-sweep`, which grows the domain-groups dictionary with the operator and refiles what accumulated.
