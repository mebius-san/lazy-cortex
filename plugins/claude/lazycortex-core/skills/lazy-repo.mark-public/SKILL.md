...
argument-hint: "[scope-glob ...]  # optional; e.g. 'plugins/claude/** README.public.md .gitignore' for subtree-public mode"
...
- **Subtree-public mode** (one or more glob args, e.g. `plugins/claude/** README.public.md .gitignore`): only the listed paths are treated as the public surface. The repo stays private on GitHub; Step 5 is skipped.

Record the chosen scope for use in Steps 2 and 4. Confirm the interpretation back to the user in one line before continuing (e.g., "Scoped mode — auditing `plugins/claude/**` only; repo visibility will NOT change").
