...
def in_repo_plugin_roots(repo: Path | None) -> list[Path]:
  """
  Return every immediate child of the repository's `plugins/claude/` directory.

  Args:
    repo: Absolute path to the repository root, or `None` when no repository was detected.

  Returns:
    The list of `<repo>/plugins/claude/<plugin>/` directories, in filesystem order. Empty when no
    repository was supplied or the `plugins/claude/` directory is absent.
  """
  # guard: no repository context — nothing to enumerate
  if repo is None:
    return []
  # waiver: filesystem path/filename idiom, not a domain constant
  candidate = repo / "plugins/claude"

  # guard: repository must own a `plugins/claude/` directory
