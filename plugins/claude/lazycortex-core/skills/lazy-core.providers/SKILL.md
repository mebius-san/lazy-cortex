...
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-get providers)
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-get providers --scope local)
```

The first call prints the `tracked` view, the second the `local` overlay, each as the section's JSON. Keep both views distinct — the entries reported under `tracked` are a `WARN`: `providers` is a machine-fact section and a tracked entry means someone committed a base URL/token-variable name into shared config by hand. Build a merged-by-name map for Steps 4–6 (local wins over tracked per key, per the section's own merge rule), noting each key's source.
...
3. **Reference check** — load the merged `experts` section (`Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-get experts --scope merged)`) and scan every entry's `provider` field for a match. Any hit is informational, never blocking: report `WARN: expert <key> still references provider <name>` for each (both placeholders substituted, not literal).
4. Confirm:
...
1. **Structural checks** — run the core CLI's `provider-validate` verb (it calls `provider_env.validate_entry`, the same disk-free function `resolve_provider` calls at dispatch time), so the wizard's checks can never drift from the runtime's:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" provider-validate '<name>' --entry '<entry-json>')
```

   The verb prints a JSON list of error strings (`[]` when valid). Any non-empty result → FAIL, list every message verbatim, no write.

2. **Token variable presence** — run the core CLI's `provider-token` verb (it reuses `provider_env.resolve_token`, same env-then-`~/.claude/.env` precedence the runtime uses) rather than re-implementing the lookup. It prints a presence verdict only — never the token itself, which would otherwise sit in the tool-call transcript:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" provider-token '<token_env>')
```

   `missing` → FAIL: token variable `<token_env>` not found in the environment or `~/.claude/.env`.

3. **Liveness probe** — resolve the token inline via command substitution (`provider-token --print`) so the literal value never appears in the Bash call or its output:

```
Bash(curl -sS -m 10 -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" provider-token '<token_env>' --print)" '<base_url>/v1/models')
```
...
2. Read-modify-write, touching only the `providers` key — every other section in the local overlay file is preserved untouched. For `add`/`update`:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-set providers --key '<name>' --value '<entry-json>' --scope local)
```

   For `remove`:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-set providers --key '<name>' --delete --scope local)
```

   The verb prints `{"status":"written",...}`; with `--scope local` it creates the local overlay file when absent and never touches the tracked file.
