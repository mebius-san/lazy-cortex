...
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" recover halt)
```
...
Run the core `recover cleanup` verb with the chosen mode; pass `--message` only for `commit` (prints `cleaned`, or `error: <reason>` with exit 1):

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" recover cleanup <mode> --message '<message>')
```
...
Skip resume if Step 2 was `aborted`. Otherwise run the core `recover resume` verb:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" recover resume)
```

It prints `resumed`, or `still-dirty: <reason>` and exits 1. If output starts with `still-dirty`: print the message and tell the operator to inspect manually (`git status`) and re-run the skill. The halt block remains.
