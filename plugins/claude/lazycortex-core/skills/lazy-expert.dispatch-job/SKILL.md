...
First confirm the expert entry exists:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-get experts --key '<expert_name>')
```

If it prints `null` → abort with outcome `aborted-no-experts-entry` and the message "`<expert_name>` is not registered in `lazy.settings.json[experts]`."

Then run the core `dispatch-job` verb with the job bundle on stdin — `expert`, `payload`, `protocols` (a JSON array, `[]` when empty), plus whichever of the io keys `source` / `context` / `source_inline` / `context_inline` / `result` the caller passed:

```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" dispatch-job <<'EOF'
{"expert": "<expert_name>", "payload": <payload-json>, "protocols": <protocols-json>, "source": [...], "context": [...], "source_inline": {...}, "context_inline": {...}, "result": [...]}
EOF
)
```

Capture and parse the JSON output: `{job_id, queue_path}` on a fresh dispatch, `{job_id, status: "already-queued"}` on a dedup hit, `{"error": "…"}` with exit 1 on a bundle parse or dispatch failure.
...
Outcome: `dispatched`, `aborted-no-experts-entry` (when the expert is not in `lazy.settings.json[experts]`), or `error`. Before dispatching, the skill must verify the expert entry exists; if `settings-get` prints `null`, abort with `aborted-no-experts-entry` and the message "`<expert_name>` is not registered in `lazy.settings.json[experts]`."
...
- **Python `FileNotFoundError` or `ModuleNotFoundError`** — `${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core` is absent or its sibling modules are missing → verify the plugin is installed (`/lazy-core.install`) and `${CLAUDE_PLUGIN_ROOT}` resolves correctly.
...
- **`{"error": "bundle parse: …"}`** — the stdin bundle is not one JSON object: `protocols` given as something other than a JSON array, an io key as something other than a JSON list / object → write `[]` / `{}` for the empty cases, or a JSON literal like `["plug:proto"]` / `{"source": ["docs/spec.md"]}`.
