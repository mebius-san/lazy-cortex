...
- **Local tool mode** — this repo *authors* plugins. Detected by `Bash(ls plugins/claude/*/.claude-plugin/plugin.json)` printing any path. Every content check also applies to plugin sources under `plugins/claude/**` (see per-agent scope expansions below). Outdated-plugin suppression (Phase 2.6) is **disabled** — the sources are authored here, so full integrity is required regardless of installed-plugin currency.
...
- **Code embedded in an instruction** — `Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/inline_code_scan.py" <repo-root>)`, adding `--dev` in local tool mode so the plugin sources under `plugins/claude/*/` are read too. The worker prints a JSON list of `{path, line, kind, text}`; every entry is a skill, agent, command, rule or reference line that carries a program instead of naming one on disk — an interpreter handed its program inline (`kind: interpreter`), a shell script in a fence or a `Bash(…)` call (`kind: shell-script`), or a program fence in a skill body (`kind: code-fence`). Emit `[FAIL] <path>:<line> embeds code (<kind>) — move it into a plugin CLI verb or a bin/ script with its own test (lazy-core.skill-writing § 13)` per entry.
- **Plugin-CLI calls bypassing the interpreter** — `Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/cli_call_scan.py" <repo-root>)`, adding `--dev` in local tool mode so the plugin sources under `plugins/claude/*/` are read too. The worker prints a JSON list of `{path, line, text}`; every entry is a skill, agent, command, rule, hook, reference or help line that runs a `lazycortex-<plugin>` CLI as a bare command (`lazycortex-core detect-scope …`), as a path-only command (`<core-bin>/lazycortex-core …`), or through a shell variable holding the path (`$WIKI_BIN apply-node …`). One `[WARN]` per entry: `<path>:<line> runs a plugin CLI without the interpreter — the file carries no exec bit and no plugin bin/ is on PATH, so the call exits 126 on any checkout a mode-blind git client touched and under every headless spawn; spell it "${LAZYCORTEX_PYTHON:-python3}" <path-to-cli> <verb>`. Entries from plugin sources carry `source: plugin`. Worker absent (older core) → `[INFO] cli_call_scan.py not shipped — skipping`.
...
- **`lazy.settings.json` validation** — process each scope (project + global) in the following order. The raw Read MUST come before the migrating helper call so that the root-`version` check sees the file before migration removes that key.
  ...
  **For each scope (project path `.claude/lazy.settings.json`, global path `$HOME/.claude/lazy.settings.json`):**
  ...
  3. **Migrating helper read (project scope only)** — for the project file, call the core CLI to obtain the migrated section dict (the verb reads through `load_section`) and trigger on-disk migration if the file is still in the old format: ``` Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" settings-get agent_models) ``` Do NOT add a manual file-existence guard — the verb prints a stub with `_version` intact when the file is absent.
...
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" provider-validate '<name>' --entry '<entry-json>')
```

    The verb prints a JSON list of error strings. `[]` → entry structurally sound. Each message in the list maps to one finding code by substring, in order:
...
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" routine-validate)
```
...
| `lazy-review.scan` | `lazycortex-core:lazy-core.markdown-style` |
  ...
  - **Coordinator-owned fix**, mechanically derivable: `Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" add-protocols --routine <name> --ids <id>)`. The union only appends, so an operator's own additions survive.
...
**Local tool mode scope expansion**: when the coordinator reports local tool mode, also grep for the same hardcoded-path patterns across `plugins/claude/**/rules/*.md`, `plugins/claude/**/skills/*/SKILL.md`, `plugins/claude/**/agents/*.md`, `plugins/claude/**/commands/*.md`, and `plugins/claude/**/hooks/**`. Self-referential documentation of the path-hygiene rule itself (files whose purpose is to describe the bad patterns — e.g. `lazy-core.hygiene.md`, `lazy-guard.security.md`, `lazy-core.doctor/SKILL.md`, `lazy-guard.check-public/SKILL.md`) must be excluded via the `source: doc-of-rule` marker to avoid false positives.
...
3. **Refresh each referenced marketplace (live).** Read `$HOME/.claude/plugins/known_marketplaces.json` to resolve each marketplace's `source` and `installLocation`. For each:
   - If `source.source == "github"`: run `git -C <installLocation> fetch --quiet origin` with a **5-second timeout** (`timeout 5 git ...` on Linux, `gtimeout 5 git ...` on macOS with coreutils; when neither exists, skip the fetch and read the manifest already checked out). Then read the latest manifest via `git show origin/HEAD:.claude-plugin/marketplace.json`. Non-destructive — working tree untouched, only remote-tracking refs advance.
...
**Retired grouping shape (same pass, same finding style).** A file-level `git` routine that still carries `group_globs`, or a boolean `group`, is on the retired unit-of-work shape and fails validation (the daemon halts with `routine_config_invalid` naming it). Scan the merged `lazy.settings.json` for either on any `routines[<name>]` block of `type: git`. Emit one finding per routine:

- `[FAIL] retired grouping shape on `routines[<name>]` (`<key>`) — the unit of work is the `group` key: "all" | "file" | "dir" | [globs]` `fix: `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> routine-migrate --apply` (offered in Phase 4)`

The fix is the migrate verb itself, never a hand edit: `group: false` becomes `"file"`, `group: true` becomes `"dir"`, and `group_globs` is dropped so the whole-tick default takes over. Silent when none present. `<core-cli>` stands for this plugin's own `bin/lazycortex-core` file, `${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core`, run through the interpreter because the file carries no exec bit.
...
2. **Local tool mode** — `plugins/claude/<plugin>/.claude-plugin/plugin.json` exists in the current repo (the plugin is authored here; all its skills are reachable via the local path).
...
- *Run condition*: at least one file in `.claude/skills/*/SKILL.md`, `plugins/claude/*/skills/*/SKILL.md`, `.claude/agents/*.md`, or `plugins/claude/*/agents/*.md`. No local skills or agents → silent skip.
...
- *Availability*: always — the contract lives in `lazycortex-core` itself.
- *Run condition*: at least one file in `.claude/skills/*/SKILL.md` or `plugins/claude/*/skills/*/SKILL.md`. No local skills → silent skip.
...
**Resolve `<wiki-cli>` once, before the first call.** It is the wiki plugin's `bin/lazycortex-wiki` file: when this repo authors the plugin itself (`plugins/claude/lazycortex-wiki/.claude-plugin/plugin.json` exists) that is `<repo-root>/plugins/claude/lazycortex-wiki/bin/lazycortex-wiki`; otherwise `Read` `$HOME/.claude/plugins/installed_plugins.json` and take `<installPath>/bin/lazycortex-wiki` from the last `lazycortex-wiki@lazycortex` record. Hold the absolute path and run every verb as `Bash("${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> <verb> …)` — never as a bare command: the file carries no exec bit and no plugin `bin/` is on `PATH`.
...
- Missing mandatory routine protocol (auto): `Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" add-protocols --routine <name> --ids <id>)` per finding. The routine and the protocol both come from the check's own table, and the union only appends — nothing the operator attached is touched.
...
- Answers: `Delete all` — each listed directory removed now, one report line per directory; `Keep` — untouched, D8 reappears next run. Not persisted.
...
On Unregister:
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" routine-unregister '<name>')
```
The verb prints `unregistered`. Report: `unregistered routine: <name>`. If the call fails (non-zero exit, e.g. settings file not writable), surface its error text as a FAIL finding and skip the write.
...
On Re-link:
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" external-dirs apply)
```
The verb prints JSON rows `{path, status, action}`. Report one line per row as `<path>: <action>`. `not_a_symlink` and `source_missing` rows are never offered here — real content and an absent source are reported only.
...
On Derive:
```
Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-core" install-phase daemon-git)
```
