...
```
plugin: <name>
plugin_dir: plugins/claude/<name>/
range: <old-sha>..HEAD
new_version: <X.Y.Z>
date: <YYYY-MM-DD>
```
...
- Validate: `plugin_dir` starts with `plugins/claude/` and ends with `/`; `range` matches `<sha>..HEAD` or `<sha>..<sha>`; `new_version` matches SemVer; `date` matches `YYYY-MM-DD`.
...
**A changed `description:` on a shipped skill, agent, or command is always kept.** Do not put it to the heuristic — it splits the question in half and the same change then lands as a bullet for one plugin and as silence for its neighbour. The field is the routing table: it decides whether the artifact is selected for a given request, so rewording it changes what runs when the user asks for something, even though nothing the user reads or types has changed. That is a user-visible change by the only definition that matters here. One bullet naming the artifact and the occasion it now answers to.
