...
surface_sha: 3df45763acc5250beb0f1083eefe1c175801ce53b562191e8b566ad15fdf375c
---
# Troubleshooting
...
## `/lazy-core.providers` fails: unrecognised mode, unknown provider name, or gitignore missing

**Symptom**: Running `/lazy-core.providers` (or `/lazy-core.providers add|update|remove <name>`) fails immediately with "mode must be one of list|add|update|remove", "provider `<name>` is not registered", or "`.claude/lazy.settings.local.json` is not gitignored".

**Likely cause (bad mode)**: The first argument passed is something other than `list` (the default when omitted), `add`, `update`, or `remove`.

**Likely cause (provider not registered)**: `update` or `remove` named a provider that isn't in the merged registry — the tracked `providers` block plus the gitignored local overlay — usually a typo, or a provider that was already removed.

**Likely cause (gitignore missing)**: `.claude/lazy.settings.local.json` was removed from `.gitignore`, or the line was never added. This skill only ever writes provider endpoints and token-variable names into that local overlay — never into the tracked settings file — and refuses to write a machine-local fact into a path git would track.

**Fix (bad mode)**: Re-run with a valid mode — `list`, `add <name>`, `update <name>`, or `remove <name>`.

**Fix (provider not registered)**: Run `/lazy-core.providers list` to see the actual registered names (each tagged `[tracked|local]`), then retry `update`/`remove` with the correct one — or use `add` if the provider genuinely doesn't exist yet.

**Fix (gitignore missing)**: Restore the `.gitignore` entry covering `.claude/lazy.settings.local.json`, then re-run `/lazy-core.providers`.
...
## `/lazy-core.doctor` flags a routine as failing its type schema, or reports a retired grouping shape

**Symptom**: `/lazy-core.doctor` reports a FAIL like "`routine <name> does not conform to its type schema — daemon halts on read`", or more specifically "`retired grouping shape on routines[<name>] (<key>) — the unit of work is the group key: "all" | "file" | "dir" | [globs]`" against an entry in `.claude/lazy.settings.json[routines]`. The runtime daemon itself may independently halt with reason `routine_config_invalid` naming the same routine.

**Likely cause (schema mismatch)**: The routine's config doesn't match the schema for its declared `type` — an unknown field, a missing required one, or a value of the wrong shape. A broken entry is invisible until the daemon actually reads it; entries written by hand, seeded by an older plugin install, or left behind by a schema change are exactly the ones no write-time validator ever caught.

**Likely cause (retired grouping shape)**: A `git`-type routine's `group` field is still a plain boolean, or the entry still carries the retired `group_globs` key — both predate the current unit-of-work shape, which is the string `"all"` (the default), `"file"`, `"dir"`, or a list of directory globs.

**Fix (schema mismatch)**: Either hand-edit the entry in `.claude/lazy.settings.json` to match the type's schema (the finding's `detail:` names the exact field), or re-register the routine from scratch with `/lazy-routine.register --force`. `/lazy-core.doctor` only reports this finding — it never rewrites the entry for you.

**Fix (retired grouping shape)**: Run `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> routine-migrate --apply` — the migrate verb rewrites `group: false` to `"file"`, `group: true` to `"dir"`, and drops `group_globs` so the whole-tick default takes over. `<core-cli>` stands for `lazycortex-core`'s own `bin/lazycortex-core` file, run through the interpreter because the file carries no exec bit. Re-run `/lazy-core.doctor` to confirm the FAIL clears.
...
**Symptom**: `/lazy-core.audit` reports a finding like "reference `<N>` KB over the 25|50 KB budget — split per lazy-core.reference-writing § 4.1, or declare `size-waiver:` when the file is genuinely read whole" against a file under `.claude/references/*.md`, `$HOME/.claude/references/*.md`, or `plugins/claude/*/references/*.md`.
...
## `/lazy-core.doctor` flags a line as "embeds code"

**Symptom**: `/lazy-core.doctor` reports a FAIL like "`<path>:<line> embeds code (<kind>) — move it into a plugin CLI verb or a bin/ script with its own test`" against one of your own rules, skills, agents, or reference files.

**Likely cause**: The flagged line hands an interpreter its program inline instead of naming a script on disk (`kind: interpreter` — an inline `python3 -c "..."` block, for example), runs a shell script inside a fence or a `Bash(...)` call (`kind: shell-script`), or embeds a program fence directly in the skill body (`kind: code-fence`). Every LazyCortex plugin CLI verb and its own test exist precisely so an instruction can name a tested primitive instead of carrying logic that only ever gets exercised when a session happens to run it.

**Fix**: Move the flagged logic into a `bin/` script or a plugin CLI verb with its own test, and rewrite the flagged instruction to call it by name instead of embedding the program. Re-run `/lazy-core.doctor` to confirm the FAIL clears; this check is read-only and does not move the code for you.
...
**Fix**: Run `/lazy-runtime.preflight` again — it now offers a `sandbox` fix for this case; accept it, or run `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> sandbox-sync --repo-root "$PWD"` by hand to regenerate `.runtime/sandbox.settings.json` against resolved paths.
...
**Likely cause**: Claude Code defaults `allowUnsandboxedCommands` to `true` when the key is absent from the sandbox file. Under that default, a command the sandbox blocks is silently retried without confinement, subject only to the ordinary permission check — which a bare `Bash` allow entry already passes — so a confined expert spawn could still write or delete outside its granted scope on the retry. `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> sandbox-sync` now writes this key `false` whenever it is missing, but a checkout whose sandbox file predates this fix keeps the old absent (defaulting-to-`true`) state until the sync runs again.

**Fix**: Run `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> sandbox-sync --repo-root "$PWD"` — or accept the equivalent fix `/lazy-runtime.preflight` offers — to write `allowUnsandboxedCommands: false` into `.runtime/sandbox.settings.json`. If the finding persists because the key is recorded `true` on purpose, that is your own decision on record: the audit and preflight checks report it but never flip it back on your behalf — remove the key or set it to `false` yourself to clear the finding.
...
## A git-type routine skips a commit you expected it to see

**Symptom**: A `git`-type routine — including one of the daemon's own built-in git-watching routines — never fires on a commit that clearly touches files inside its `watch` mode and `path_filter`. The commit shows up fine in `git log`, but the routine's cursor moves right past it as if it carried no relevant changes at all.

**Likely cause**: The commit's message carries the trailer `Lazy-Watch: skip` at its end. Every `git`-type routine treats such a commit as invisible, regardless of `watch` mode (`new_commits`, `new_files`, `changed_files`, `deleted_files`, `renamed_files`), `path_filter`, or `group` — its items are dropped and the routine's cursor advances past it. This is a deliberate per-commit opt-out, typically used on housekeeping commits (a version bump, a folder-note rewrite, a changelog pass) that would otherwise fire every registered watcher for a change nobody wants acted on.

**Fix**: If the commit should have been watched, the trailer was added by mistake at commit time — there is no way to make a routine retroactively re-see a commit its cursor has already passed; the next commit that touches the same files fires normally. If you want a certain kind of commit to stay invisible to every git-type routine going forward, end its commit message with a trailer line reading exactly `Lazy-Watch: skip` (a blank line, then the trailer, at the very end of the message) — this is a plain git commit-message convention, not a file any skill owns.
