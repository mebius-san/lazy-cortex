...
surface_sha: 2ba1b9a3d233dd3f638e84295239f687a9551fabffd1858ffdcb60c4d0e4d597
---
...
**`/lazy-expert.dispatch-job`** is the entry point into the async team. You supply the expert name, the job payload (kind, role, and request are required; source, context, and result arrays are optional file references), and an optional list of protocol refs. The skill validates the payload, checks the expert is registered in `lazy.settings.json[experts]`, writes the job directory under `.experts/.jobs/<expert_name>/`, and returns `{job_id, queue_path}` immediately. From that point the main session is free. Dispatching the same job again under the same dedup key does not queue a duplicate — you get back the already-queued job's `job_id` (`status: "already-queued"`) instead.

**`/lazy-expert.list-jobs`** gives you a live snapshot of every job in the queue, sorted oldest-first. Each row shows expert, job_id, status, and age. The statuses map directly to what the daemon writes to disk: `queued` (READY marker present, no PID — waiting to be picked up), `active` (READY + PID — daemon is running it now), `cancelled` (a CANCELLED marker was written — outranks every other marker), `dead` (DEAD marker present — unrecoverable launch error), `done` (DONE marker + a response whose outcome is not `error` — this also covers a deferred outcome, since `deferred` is not `error`), and `failed` (DONE marker + a response whose outcome is `error`, or a response missing/malformed/without an `outcome` field). Pass `expert=<name>` or `status=<value>` — one of `queued`, `active`, `cancelled`, `dead`, `done`, `failed` — to narrow the listing; `deferred` is not one of the accepted filter values here. Use this before collecting to confirm a job has finished, or to locate a `job_id` you have lost track of.
...
When you see a job reach `done` status in the list, run `/lazy-expert.collect-job` to retrieve the output. If the result status is `pending`, the daemon is still working — call again in a moment. If it comes back `deferred`, the expert chose not to act this time and left its input exactly where it was; nothing needs cleanup on your side, and this is the only way to learn a `done`-listed job was actually deferred, since `list-jobs` folds it into `done`. If it comes back `failed`, read the error and check `transcript.jsonl` in the job directory for the full subprocess output — remember that `failed` also covers a response that simply forgot to report an `outcome`, so a blank-looking error can mean the expert's response was malformed rather than that it hit a real failure.
...
- **Filter by status.** Pass `status=queued` to see jobs waiting to be picked up, `status=active` to confirm the daemon is currently working, `status=cancelled` to review jobs you stopped, `status=dead` to surface jobs the daemon could not launch, or `status=failed` to find jobs with error or malformed outcomes. `deferred` is not one of the six accepted filter values — a deferred job appears under `status=done` in the listing, so run `/lazy-expert.collect-job` on it to see that its outcome was actually `deferred`.
...
- **Don't worry about accidental double-dispatch.** Re-sending the same job payload under the same dedup key does not create a second job — you get the original `job_id` back with `status: "already-queued"`. To force a fresh run of identical work, cancel the existing job first (releases the dedup key), then dispatch again.
