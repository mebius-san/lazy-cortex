...
surface_sha: a4af9591cb12ec0f1337404b015fc0a6ab081c8541c50a0537e8580d3248ad39
---
...
- **git** — watch local HEAD for new commits, new files, changed files, deleted files, or renamed files; fire once per item. Required: `watch`, `interval_sec`. The `branch` and `remote` fields are vestigial — the watch always targets local HEAD regardless of their values, and remote sync is the daemon's own job. The wizard may surface them for schema compatibility; leave them blank or skip them. `repo_dir` defaults to `.` — change it only when the routine should watch a different checkout than the one the daemon runs in. Optional: `filter` (the same composite frontmatter block `md-scan` takes, for watched paths worth narrowing by frontmatter), `path_filter` (a plain path-glob narrowing before the frontmatter filter runs), `group` (the unit of work, default `"all"` — every changed path of a tick is folded into one run carrying `dir: "."` and the `paths` list, for `command` and `expert` routines alike; `"file"` fires once per changed file, `"dir"` once per parent directory, and a list of directory globs once per matched directory, the deepest matching glob winning). A commit whose message ends with the git trailer `Lazy-Watch: skip` never produces an item for any `git` routine, regardless of `watch` mode or `group` setting — its changes are dropped, a path it last touched is re-attributed to the newest ordinary commit that changed it the same way, and the watch cursor still advances past it. Use the trailer on your own commits (a comment sweep, a formatting pass, a mechanical rename) when no `git` routine should react to them.
...
For a `git` routine, supply `watch` (one of `new_commits` / `new_files` / `changed_files` / `deleted_files` / `renamed_files`) and `interval_sec`. The `branch` and `remote` fields are accepted by the schema for compatibility but have no effect on which changes the watch observes — skip them. The `group` key sets the unit of work and defaults to `"all"`: every changed path of a tick is folded into one run carrying `dir: "."` and the `paths` list, whichever dispatch shape the routine uses. Set `"file"` for one run per changed file, `"dir"` for one per parent directory, or a list of directory globs (e.g. all the parts of one asset living under `specs/*/*`) for one run per matched directory. Whatever `watch` or `group` you pick, a commit carrying the `Lazy-Watch: skip` trailer at the end of its message stays invisible to this routine — no run fires for it, and the watch cursor moves past it as if it never happened. Add that trailer to your own commits when you want a mechanical or narration-only change to bypass every `git` routine watching the repo.
