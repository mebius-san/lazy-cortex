...
Optional fields — `source` (array of input file paths), `context` (array of context file paths), and `result` (array of expected output paths) — are supported but not required. Protocol-specific extras are also allowed. The full contract is in `plugins/claude/lazycortex-core/references/lazy-core.expert-protocols-contract.md`.
...
## How many jobs does a `git` routine dispatch when many files change at once?

One, unless the routine says otherwise. The unit of work of a file-level `git` routine (`watch: changed_files` / `new_files` / `deleted_files` / `renamed_files`) is its `group` key, and with the key absent the whole tick is one unit: every changed path reaches the expert as one job, or the command worker as one call, carrying `dir: "."` and the full `paths` list (plus `old_paths` on a rename watch). A tree-wide rename therefore costs one job, not one per directory. Set `group: "file"` for one unit per changed file, `group: "dir"` for one per parent directory, or a list of directory globs for one per matched directory — with globs, the deepest matching glob wins and list order carries nothing. A `new_commits` watch is always one unit per commit and rejects the key. A registration still carrying the retired `group_globs` key or a boolean `group` fails validation; `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> routine-migrate --apply` rewrites it.

---

## How do I stop a `git` routine from reacting to one specific commit?

Add the git trailer `Lazy-Watch: skip` to the commit message (a trailer line at the end of the body; the value is matched case-insensitively) before you commit. Every `git`-type routine treats that commit as invisible, regardless of its `watch` mode (`new_commits`, `new_files`, `changed_files`, `deleted_files`, `renamed_files`) or dispatch shape (`command` or `expert`): a `new_commits` watch drops the commit as an item outright; a file-level watch re-keys any path the marked commit was the last to touch to the newest unmarked commit in the range that also touched it, and drops a path only marked commits ever changed. The routine's cursor still advances past the skipped commit, so a later tick never revisits it.

This is meant for a mechanical mass commit that shouldn't fire any watcher — applying one comment template across a catalog, a repo-wide formatting sweep, a bulk rename — not for hiding day-to-day work from routines you actually want reacting to it. Any other trailer value, or no trailer at all, leaves the commit visible as before.
