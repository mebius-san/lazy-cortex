...
# the three named `group` modes; a list value names directory globs instead
GROUP_ALL = "all"
GROUP_FILE = "file"
GROUP_DIR = "dir"

# the `dir` a whole-tick unit carries: the repository root
GROUP_ROOT_DIR = "."

# the retired directory-glob key — named in the migration error, rewritten by `routine-migrate`
LEGACY_GROUP_GLOBS_KEY = "group_globs"

# the stdout key a command worker names its unhandled paths under, narrowing the retry unit
FAILED_PATHS_KEY = "failed_paths"

# the commit-message trailer that hides a commit from every git watch: `Lazy-Watch: skip`
SKIP_TRAILER_KEY = "Lazy-Watch"
...
def validate_routine_entry(name: str, cfg: dict) -> None:
  ...
  # guard: the retired glob key names the migration instead of the generic unknown-field message
  # waiver: routine-type token, single-source set in VALID_TYPES/SCHEMAS, not a reusable cross-module key
  if rtype == "git" and LEGACY_GROUP_GLOBS_KEY in keys:
    raise RoutineConfigError(
      f"routine '{name}' (type=git): '{LEGACY_GROUP_GLOBS_KEY}' was retired — the unit of work is "
      f"'{GROUP_KEY}' (\"{GROUP_ALL}\" | \"{GROUP_FILE}\" | \"{GROUP_DIR}\" | [<glob>, …]); "
      f"run `lazycortex-core routine-migrate --apply` to rewrite the registration"
    )
  ...
  if rtype == "git":
    ...
    # the unit of work is one key with a closed vocabulary; absent means the whole tick
    group = cfg.get(GROUP_KEY)
    if group is not None:
      # guard: a boolean is the retired opt-out shape — name the migration rather than a type error
      if isinstance(group, bool):
        raise RoutineConfigError(
          f"routine '{name}' (type=git): '{GROUP_KEY}' is no longer a boolean — write "
          f"\"{GROUP_FILE}\" for one unit per file or \"{GROUP_DIR}\" for one per directory; "
          f"run `lazycortex-core routine-migrate --apply` to rewrite the registration"
        )
      # guard: a string names one of the three modes, a list names non-empty directory globs
      if not (
        (isinstance(group, str) and group in (GROUP_ALL, GROUP_FILE, GROUP_DIR))
        or (isinstance(group, list) and group
            and all(isinstance(glob_pat, str) and glob_pat.strip() for glob_pat in group))
      ):
        raise RoutineConfigError(
          f"routine '{name}' (type=git): '{GROUP_KEY}' must be \"{GROUP_ALL}\", "
          f"\"{GROUP_FILE}\", \"{GROUP_DIR}\" or a non-empty list of directory globs"
        )
      ...
      if watch == "new_commits":
        raise RoutineConfigError(
          f"routine '{name}' (type=git): '{GROUP_KEY}' cannot be combined with "
...
def dispatch_routine(repo: Path, name: str, cfg: dict) -> dict:
  ...
  if rtype == "subprocess":
    ...
    # waiver: bare-name sibling import (flat bin/), resolved at runtime via sys.path; not statically resolvable
    from runtime_daemon import dispatch_subprocess  # pylint: disable=import-error
...
def dispatch_inbox(repo: Path, name: str, cfg: dict) -> dict:
  ...
  if use_command:
    ...
    try:
      # waiver: deferred / late-bound local import per the plugin import style (avoids import cycles / optional deps)
      # waiver: bare-name sibling import (flat bin/), resolved at runtime via sys.path; not statically resolvable
      from runtime_daemon import resolve_routine_command  # pylint: disable=import-error
...
def due_for_schedule(name: str, cfg: dict, now_unix: float, last_run_unix: float) -> bool:
  ...
  # waiver: deferred / late-bound local import per the plugin import style (avoids import cycles / optional deps)
  # waiver: bare-name sibling import (flat bin/), resolved at runtime via sys.path; not statically resolvable
  from cron import parse, due_since  # pylint: disable=import-error
  spec = parse(cfg["cron"])
  # waiver: inline numeric/default literal, not a domain constant
...
def dispatch_git(repo: Path, name: str, cfg: dict) -> dict:
  ...
  Guarantees:
    ...
    - A commit whose message carries the `Lazy-Watch: skip` trailer contributes no item to any
      watch; a path it changed is judged by the newest ordinary commit of the range that changed the
      path the same way, or not at all.
  ...
  if not force_pushed and last_seen != head_sha:
    ...
    # a commit the author marked with the skip trailer is invisible to the watch: its own items
    # go, and a path it was the last to touch is re-keyed to the newest unmarked commit instead
    if fresh_items:
      fresh_items = _drop_skipped_commits(work_dir, fresh_items, f"{last_seen}..{head_sha}")
    ...
    # the unit of work — the `group` mode folds the surviving file-level items AFTER the filter
    # and the self-author drop, so a worker is invoked once per unit over its members. The
    # default is the whole tick as one unit, the one shape that cannot fan out; a consumer that
    # wants a unit per file, per directory, or per declared directory glob says so.
    # waiver: git watch-mode token, single-source set in VALID_GIT_WATCH, not a reusable cross-module key
    mode = cfg.get(GROUP_KEY, GROUP_ALL)
    if fresh_items and watch != "new_commits" and mode != GROUP_FILE:
      fresh_items = _group_git_items(work_dir, fresh_items, mode, f"{last_seen}..{head_sha}")
    ...
    for item in retry_items + fresh_items:
      ...
      if proc.returncode != 0:
        ...
        still_failing.append({
          **_narrow_failed_item(clean_item, proc.stdout),
          "reason": f"exit {proc.returncode}: {stderr_tail}",
        })
...
def _git_dedup_key(routine: str, item: dict) -> str | None:
  ...
  Returns:
    A stable key naming the unit directory (`"."` for a whole-tick unit) or the changed path,
    or None for a watch whose items carry no path at all.
  """
...
def _group_dir_for(path: str, mode: str | list) -> str | None:
  """
  Resolve the unit directory a file path collapses into under one `group` mode, if any.

  The whole-tick mode claims every path for the repository root. The directory mode claims
  a path for its own parent directory, so a root-level file has no unit directory. With a
  glob list, the deepest matching glob wins; two globs of equal depth name the same directory
  prefix, so list order carries nothing. A glob matches segment-by-segment (`*` never crosses
  `/`), and the file must sit strictly below the glob's depth, so a note lying AT that depth
  (a folder-note beside the group dirs) never becomes its own group.

  Args:
    path: Repo-relative file path from a git-watch item.
    mode: The routine's `group` value — `"all"`, `"dir"`, or a list of directory globs; the
      per-file mode never reaches here.

  Returns:
    The unit directory as a repo-relative path (`"."` for the whole-tick mode), or None when
    no glob matches and for a root-level file in the directory mode.
  """

  # Domain(runtime.routines):
  # # The unit of work of a file-level watch
  # A git watch sees one changed file at a time, but the work it wakes is rarely about one file.
  # The unit it dispatches is therefore a declared choice with four shapes: the whole tick as one
  # unit, one unit per changed file, one unit per directory a changed file sits in, or one unit per
  # directory a declared glob names. The deepest glob that matches wins — two globs of equal depth
  # name the same directory prefix, so the declaration is a set and its order carries nothing —
  # matching is done segment by segment so a wildcard never crosses a path separator, and a file
  # must sit strictly below the matched glob's own depth — a file that sits exactly at that depth,
  # such as a note describing the group itself, never becomes a group of one. A watch whose items
  # carry no path at all has nothing to fold and always dispatches one unit per item.
  #
  # Declaring nothing means the whole tick is one unit, because that is the one shape that cannot
  # multiply work: a consumer that omitted the choice receives one call or one job per tick and
  # decides for itself how much work that is, and a tree-wide change reaches it as a single batch
  # rather than as one unit per directory. What a consumer omitted from its configuration must
  # never decide how much the work costs.

  # the whole-tick mode: every path belongs to the root unit
  if mode == GROUP_ALL:
    return GROUP_ROOT_DIR

  # the directory mode: the file's own directory is the unit; a root-level file has none
  if mode == GROUP_DIR:
    # waiver: filesystem path idiom, not a domain constant
  ...
  for pattern in mode:
    ...
    # guard: a non-matching glob claims no group dir for this file
    if not all(fnmatch.fnmatch(seg, p) for seg, p in zip(parts[: len(pat)], pat, strict = True)):
      continue

    # the deepest matching glob claims the file; equal-depth globs name the same prefix anyway
    if len(pat) > best_depth:
      best, best_depth = "/".join(parts[: len(pat)]), len(pat)
  return best


def _group_git_items(work_dir: Path, items: list[dict], mode: str | list, rng: str) -> list[dict]:
  """
  Collapse file-level items into one item per unit directory of the `group` mode.

  A worker whose real unit of work is a directory (an asset folder, a bundle) or the whole
  tick is invoked once per unit with every member path, instead of once per file. Items whose
  path resolves to no unit directory pass through unchanged, so grouping is an overlay on the
  file-level watch, not a replacement.
  ...
  members' `status` when every member shares it.

  Args:
    work_dir: Path-like reference to the git working tree.
    items: File-level items from `_compute_git_items`, already filtered.
    mode: The routine's `group` value — `"all"`, `"dir"`, or a list of directory globs.
    rng: The `<last_seen>..<head>` range this tick scans, for the group's attribution.

  Returns:
    Ungrouped items followed by one dict per unit directory, each carrying `dir`, sorted
    member `paths`, and the `sha` / `author_name` / `author_email` of the last commit
    touching the unit in range (the range's last commit for the whole-tick unit).
  """
  # waiver: deferred / late-bound local import per the plugin import style (avoids import cycles / optional deps)
  import subprocess

  # split the stream: path-less or unmatched items pass through, the rest bucket by unit dir
  singles: list[dict] = []
  ...
  for item in items:
    ...
    gdir = _group_dir_for(path, mode) if path else None
    if gdir is None:
      singles.append(item)
      continue
    groups.setdefault(gdir, []).append(item)

  # emit one item per unit dir, appended after the pass-through singles
  for gdir in sorted(groups):
    members = groups[gdir]

    # attribute the unit to the last commit touching it in range — one log call per unit; the
    # root unit is the whole range, so no pathspec narrows the log for it
    pathspec = [] if gdir == GROUP_ROOT_DIR else [ "--", gdir ]
    try:
    ...
    # a status is carried only when every member shares it — the whole-tick unit of a
    # changed_files watch may mix additions and modifications, and a wrong status is worse than none
    # waiver: `_compute_git_items`'s per-watch item-shape key, not a reusable domain key
    if len({ member.get("status") for member in ordered }) == 1 and ordered[0].get("status"):
      # waiver: `_compute_git_items`'s per-watch item-shape key, not a reusable domain key
...
def _narrow_failed_item(item: dict, stdout: str) -> dict:
  """
  Narrow a failed multi-path unit to the paths its worker reported as unhandled.

  A worker that handled part of a unit names the rest under `failed_paths` in a JSON object
  on the last line of its stdout; the retry entry then carries only those members, so the
  paths that succeeded are not replayed.

  Guarantees:
    - The retry entry is narrowed to the reported members only when the last non-blank stdout
      line is a JSON object whose `failed_paths` names at least one member of the unit; the
      unit is retried whole otherwise.
    - A narrowed rename unit keeps `old_paths` aligned index by index with `paths`.

  Args:
    item: The unit item the worker was spawned with, as sent.
    stdout: The worker's captured stdout.

  Returns:
    The item narrowed to the reported paths (with `old_paths` narrowed index-aligned), or the
    item unchanged.
  """

  # Contract:
  # The retry entry is narrowed to the reported members ONLY when the last non-blank stdout
  # line is a JSON object whose `failed_paths` names at least one member of the unit; with no
  # such line, a non-JSON line, or a report naming none of the members, the unit is retried
  # whole. A narrowed rename unit keeps `old_paths` aligned index by index with `paths`.

  # waiver: deferred / late-bound local import per the plugin import style (avoids import cycles / optional deps)
  import json as _json

  # the last non-blank stdout line is where the protocol puts the object; anything else is noise
  lines = [ line for line in (stdout or "").splitlines() if line.strip() ]

  # guard: no output at all — nothing to narrow by
  if not lines:
    return item

  # the protocol object is whatever the last line parses to; a non-JSON line is not the protocol
  try:
    reported = _json.loads(lines[-1])
  except ValueError:
    # guard: the last line is not JSON — the worker did not speak the protocol
    return item

  # guard: not the protocol's object shape — retry the unit whole
  if not isinstance(reported, dict) or not isinstance(reported.get(FAILED_PATHS_KEY), list):
    return item

  # the unit's members are what the report is matched against
  # waiver: `_compute_git_items`'s per-watch item-shape keys, not reusable domain keys
  members = item.get("paths")

  # guard: a single-path item has no members to narrow
  if not isinstance(members, list):
    return item

  # keep the members the worker named, in the unit's own order
  unhandled = set(reported[FAILED_PATHS_KEY])
  keep = [ index for index, member in enumerate(members) if member in unhandled ]

  # guard: the report names none of the unit's members — the whole unit failed as far as we know
  if not keep:
    return item

  # the narrowed unit carries only the unhandled members
  narrowed = { **item, "paths": [ members[index] for index in keep ] }

  # a rename unit keeps both sides aligned
  # waiver: `_compute_git_items`'s per-watch item-shape keys, not reusable domain keys
  old_paths = item.get("old_paths")
  if isinstance(old_paths, list) and len(old_paths) == len(members):
    # waiver: `_compute_git_items`'s per-watch item-shape key, not a reusable domain key
    narrowed["old_paths"] = [ old_paths[index] for index in keep ]
  return narrowed


def _query_skipped_shas(work_dir: Path, rng: str) -> set[str]:
  """
  Return the commits in `rng` whose message carries the `Lazy-Watch: skip` trailer.

  Notes:
    - Spawns one `git log` subprocess over the range.

  Args:
    work_dir: Path-like reference to the git working tree.
    rng: The `<last_seen>..<head>` range this tick scans.

  Returns:
    The full shas of the marked commits; empty when none is marked or the log fails.
  """
  # one log call reads every commit's trailer value; git resolves the trailer block itself
  try:
    out = subprocess.check_output(
  ...
  # a commit is marked when any occurrence of the trailer spells the skip value
  marked: set[str] = set()
  for line in out.splitlines():
    sha, _tab, values = line.partition("\t")
    if any(value.strip().lower() == SKIP_TRAILER_VALUE for value in values.split(",")):
      marked.add(sha)
  return marked


def _drop_skipped_commits(work_dir: Path, items: list[dict], rng: str) -> list[dict]:
  """
  Remove the work a skip-marked commit contributed to a tick's items.

  Guarantees:
    - An item whose commit is not marked is returned unchanged, in its original position.
    - A commit-level item of a marked commit is dropped.
    - A path-level item whose commit is marked is re-keyed to the newest unmarked commit of
      the range that changed the path the same way, and dropped when there is none.
    - A path-level item whose fallback lookup fails is returned unchanged rather than dropped.

  Notes:
    - Spawns one `git log` subprocess over the range, plus one per path item a marked commit
      owns.

  Args:
    work_dir: Path-like reference to the git working tree.
    items: The tick's items from `_compute_git_items`, each carrying its commit as `sha`.
    rng: The `<last_seen>..<head>` range this tick scans.

  Returns:
    The items minus those a marked commit owns, the re-keyed survivors in their original
    positions.
  """

  # Contract:
  # An item whose `sha` is not a marked commit is returned unchanged, in its original position.
  # A commit-level item of a marked commit is dropped. A path-level item whose `sha` is a marked
  # commit is re-keyed to the newest unmarked commit of the range that changed its path with the
  # same diff status (added / modified / deleted / renamed), and dropped when no such commit
  # exists. A path-level item whose fallback lookup fails is returned unchanged, never dropped.

  # Domain(runtime.routines):
  # # A commit the author declared invisible to the watches
  # Some commits change many files without changing what any watcher cares about: a comment
  # template applied across a catalogue, a mechanical rename, a formatting sweep. Their author
  # knows that at commit time, and says so with a trailer on the commit message. Every git watch
  # treats such a commit as if it had never happened: the commit itself wakes nothing, and a file
  # it touched is judged only by the ordinary commits of the same range that changed the file the
  # same way — a file only the marked commit added, deleted or edited wakes nothing, a file an
  # ordinary commit also changed is attributed to that commit. The cursor still moves past the
  # marked commit, so the declaration is a skip, never a deferral.

  # the marked set is read once per tick; nothing marked means nothing to drop
  marked = _query_skipped_shas(work_dir, rng)

  # guard: nothing marked in the range — the items stand as computed
  if not marked:
    return items

  # limit: one `git log` per path item a marked commit owns — a marked sweep over N files costs N
  # log calls on the tick that sees it; upgrade path is one `git log --name-status` over the
  # range, bucketed by path, if such sweeps become frequent enough to measure
  kept: list[dict] = []
  for item in items:
    # waiver: `_compute_git_items`'s per-watch item-shape keys, not reusable domain keys
    if item.get("sha") not in marked:
      kept.append(item)
      continue

    # a marked commit's own item survives only through a path an ordinary commit also changed
    # waiver: `_compute_git_items`'s per-watch item-shape keys, not reusable domain keys
    path = item.get("path") or item.get("new_path")

    # guard: a commit item of a marked commit has no path to fall back on — it is gone
    if not path:
      continue

    # the newest unmarked commit that changed the path the same way, walking the range's log
    # waiver: `_compute_git_items`'s per-watch item-shape key, not a reusable domain key
    flag = _DIFF_FILTER_FLAGS.get(str(item.get("status")), _DIFF_FILTER_FLAGS["R"])
    try:
      ...
      kept.append(item)
      continue
    fallback = next((line for line in log.splitlines() if line.split("\t", 1)[0] not in marked), None)

    # guard: only marked commits changed the path that way in range — the item is gone
    if fallback is None:
      continue

    # the log line is sha, name, email; padding covers a commit with an empty identity field
    sha, author_name, author_email = [ *fallback.split("\t"), "", "" ][:3]
    # waiver: `_compute_git_items`'s per-watch item-shape keys, not reusable domain keys
    kept.append({ **item, "sha": sha, "author_name": author_name, "author_email": author_email })
  return kept
...
def _walk_repo_files(repo: Path, now: float) -> list[Path]:
  ...
  # waiver: deferred / late-bound local import per the plugin import style (avoids import cycles / optional deps)
  # waiver: bare-name sibling import (flat bin/), resolved at runtime via sys.path; not statically resolvable
  from repo_walk import RepoWalk  # pylint: disable=import-error
