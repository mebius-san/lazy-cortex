...
under `plugins/claude/*/` and the root READMEs. Markdown, YAML and shell files are read; Python and
...
_LOCAL_ROOT, _DEV_ROOT = ".claude", "plugins/claude"
...
def _candidates(root: Path, dev: bool) -> list[Path]:
  ...
  Args:
    root: Repo root.
    dev: Whether the repo authors plugins and `plugins/claude/*/` joins the scope.
