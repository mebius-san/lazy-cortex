...
|---|---|---|
| Plugin-shipped | `plugins/claude/<plugin>/references/<name>.md` | `<plugin>:<name>` via `bin/reference_resolver.py` |
...
`lazy-core.audit` runs the checks above on `.claude/references/*.md`, `~/.claude/references/*.md`, and `plugins/claude/*/references/*.md`. `lazy-core.doctor` Phase 3 surfaces the findings and prompts for fixes. Subtype-specific deeper validation (e.g. protocol §§ 4.1–4.9 from `lazy-core.expert-protocols-contract.md`) is enforced by `lazy-core.audit`'s expert-runtime phase.
