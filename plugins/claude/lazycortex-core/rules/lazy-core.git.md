...
- **A sync-displaced index heals itself.** `index (…conflicted copy…)` files beside `.git/index` mean a cloud-sync client raced git and resurrected a stale index; the `lazy-core.index-guard` routine (and the git-guard hook's pre-flight) restores the newest copy and removes the litter automatically. Never hand-copy those files, and never diagnose staged content before the heal has run.
