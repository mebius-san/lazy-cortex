...
Audience: anyone authoring a Claude Code lifecycle hook script (`PreToolUse`, `PostToolUse`, `Stop`, `SessionStart`, etc.). Applies to every hook script under `.claude/hooks/**` and `plugins/claude/*/hooks/**`.
...
A hook that auto-commits its own writes MUST have a content-based bail: `git diff-tree --no-commit-id --name-only -r --root HEAD` plus a predicate recognising this hook's own footprint (e.g., "every changed path matches `plugins/claude/<x>/<x>.md`"). On match, return 0 without re-running the work — otherwise the hook re-fires on its own commit indefinitely.
