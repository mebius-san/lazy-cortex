...
Before composing any **new** file whose path matches a glob below, `Read` the matching template first and start from it — never compose from memory. Contract & extension rules: `plugins/claude/lazycortex-core/references/lazy-core.scaffold-registry-contract.md`.
