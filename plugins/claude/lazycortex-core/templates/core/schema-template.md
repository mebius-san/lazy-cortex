...
- Placement: `<plugin>/references/<name>-schema.md` for new files (suffix triggers this template via lazy-core.scaffold). Pre-existing schema docs without the suffix would be grandfathered (per `lazy-core.reference-writing § 1`); the in-tree set was migrated and none currently remain.
- Drop the `## 1.` / `## 2.` numbering if the schema is small — see `lazy-obsidian.iconize-settings-schema.md` for a flat-table style with no subsections.
