...
| `git` | `branch`, `watch`, `interval_sec` | `repo_dir`, `remote`, `path_filter`, `filter`, `group`, `watch_runtime_trees`, `timeout_sec` |
...
its new path's index. Writing a request against `{paths}` therefore serves every `group` mode
alike, and changing the mode later changes how many units are dispatched without changing the
request's shape.
...
**`Lazy-Watch: skip` trailer.** A commit whose message carries the git trailer `Lazy-Watch: skip`
(a trailer line at the end of the body, value case-insensitive) is invisible to every git watch,
whatever its `watch` mode or dispatch shape: the commit item itself is dropped, a path the marked
commit was the last to change is re-keyed to the newest unmarked commit of the range that touched
it, and a path only marked commits changed dispatches nothing. The cursor still advances past the
commit. It is the author's declaration for a mechanical mass commit — a comment template applied
across a catalogue, a formatting sweep, a rename — that no watcher should react to; any other
trailer value leaves the commit visible.
...
**`group`** (optional, file-level watches only — rejected with `new_commits`, whose items are
always one per commit) — the unit of work one tick dispatches. It governs both dispatch shapes
the same way: an `expert` routine queues one job per unit, a `command` routine spawns the worker
once per unit with the unit's item as its JSON argv. One key, four modes:

| `group` | Unit | Item shape |
|---|---|---|
| `"all"` (default when absent) | Every surviving item of the tick as ONE unit. | `{dir: ".", paths, sha, author_name, author_email}` plus `old_paths` on a rename watch and `status` when every member shares one. |
| `"file"` | One unit per changed file. | The plain file-level item (`{path, paths, status, sha, …}`). |
| `"dir"` | One unit per parent directory of the changed files; a root-level file is its own unit. | `{dir, paths, sha, author_name, author_email}` per directory. |
| `[<glob>, …]` | One unit per directory a glob matches; a path outside every glob stays a file-level unit. | `{dir, paths, …}` per matched directory. |

The default is the whole list because that is the one unit that cannot fan out: a consumer
that omitted the key gets one call or one job per tick and decides for itself how much work
that is — a whole-tree rename reaches a curator as a single batch instead of one job per
directory. A consumer that genuinely wants a job per file says `"file"`; a consumer whose unit
is a folder says `"dir"` or names the folder globs. With a glob list the deepest matching glob
wins — the list is a set, not an ordering; a glob matches segment-by-segment (`*` never crosses
`/`); a file lying AT the glob's depth (a folder-note beside the group dirs) and any path
outside every glob stay ordinary file-level items. A grouped item's `{sha}` /
`{author_name}` / `{author_email}` name the last commit touching its `dir` in the scanned
range (the range's last commit for `"all"`). The unit is also the retry unit of a `command`
routine (below).

Any other value — a boolean, an unknown string, an empty or ill-typed list — is a
`RoutineConfigError`, and so is the retired `group_globs` key: `"${LAZYCORTEX_PYTHON:-python3}" <core-cli> routine-migrate --apply`
(`<core-cli>` stands for the core plugin's `bin/lazycortex-core` file, run through the interpreter, never bare)
rewrites a registration that still carries the old shape (`group: false` → `"file"`,
`group: true` → `"dir"`; `group_globs` is dropped so the whole-tick default takes over — a
consumer that really wants a unit per named directory declares the list again under `group`).
...
**`command`-shape retry cursor.** A unit whose spawned worker exits non-zero is recorded in `git_watch.<name>.failed_items` (`{sha, ..., reason}` — the full item dict plus a bounded `reason` string carrying the exit code and a trimmed stderr tail) instead of being silently dropped; `last_seen_sha` still advances past it so one broken unit never stalls the rest of the range. The next tick retries every `failed_items` entry — in order, before any fresh unit — and clears an entry on success; a repeat failure leaves it in place (no retry cap: a permanently failing unit is a permanently visible line in the daemon's routine-tick journal, the operator's diagnostic). An entry whose `sha` history no longer knows (a later force-push rewrote it away) is dropped instead of replayed, using the same ancestry check the baseline-reset guard uses; the tick's `note` field reports the drop. This applies only to the `command` sub-shape — the `expert + request` sub-shape dispatches jobs asynchronously and has no synchronous exit code to observe.

**Partial failure of a multi-path unit.** A worker that handled some of a unit's `paths` and failed on others narrows what is retried by printing, as the LAST line of its stdout, a JSON object carrying `failed_paths` — the subset of the unit's `paths` it could not handle — and exiting non-zero. The retried entry then carries only those paths (with `old_paths` narrowed index-aligned) instead of the whole unit, so the paths that succeeded are not replayed. A non-zero exit with no such line, or a `failed_paths` naming no path of the unit, retries the unit whole.
...
**`filter.folder_note`** (tri-state) — constrains matches by folder-note status. A file `p` is a folder note iff `Path(p).stem == Path(p).parent.name` (e.g. `plugins/claude/lazycortex-core/lazycortex-core.md`). Obsidian plugin settings are never consulted; the convention is hardcoded.
