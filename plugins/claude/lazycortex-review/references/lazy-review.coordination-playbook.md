...
**The repaint moves the colour and never the icon.** A document's icon says what the document IS; a review phase and a review verdict are states, and every matcher in `references/lazy-review.iconize-registry.json` is a state matcher. So none of them resolves an icon of its own — each writes `"{{frontmatter.iconize_icon}}"`, borrowing whatever the document already carries, and pairs it with the colour that phase deserves. A document under review keeps the icon its own plugin gave it and only changes colour as it moves; the full rule is `lazycortex-obsidian:lazy-obsidian.iconize-registry-contract.md` § 3a.
...
**The state the ladder runs on.** The closed frontmatter schema: `review_active`, `review_round`, `review_approved`, `review_phase`, `review_main_done`, `review_validation_round`, `review_approved_with_concerns`, `review_result`, `review_expert`, `review_marker_style` — ten keys, all of them content-facing. The last is the cycle's edit-marker pin, seeded by the entry verb and read by every marker-aware consumer afterwards (Chapter 4): it is inside the schema, so `set-key` accepts it and `parse-note` reports it as ordinary frontmatter rather than as an `unknown_review_keys` signal — but a pin the entry verb already seeded is one the coordinator has no reason to rewrite. The two job markers are deliberately not among them (Chapter 1); they reach the same `parse-note` report through its `job_markers` block. The phase is **explicit frontmatter, never derived from commit messages** — that is precisely what stops an operator commit inside a phase (answering a terminal's question, say) from re-anchoring the chain or re-opening a barrier that already closed.
...
**Section writers before approve.** A section writer is pending when its owned H1 section is *absent from the body* — a body-presence probe, never a walk of commit trailers. A pre-approve section writer runs only when no main writer is pending and no operator block stands. An operator block is an unanswered `[!question] #review/question` or an unanswered `[!decision-candidate]` (its accept/reject pair untouched); a callout counts as answered when a `- [x]` sits inside its own body. There is no concern callout: a validator's findings are plain prose in its own section, and the main writer turns them into questions with options, so nothing an expert writes can block the operator without giving them a box to tick. Two exceptions, both about not blocking a writer on its own words: a callout inside a writer's own owned section never blocks that writer (it placed the callout and is the only one who can retire it), and a terminal-owned section carrying any `- [x]` anywhere counts as answered wholesale — in a terminal section the tick *is* the answer and the callout is just the reminder of what was asked.
...
**Finalize.** The conditions, all of them: the document is approved (or approved-with-concerns), no barrier work is pending, no unanswered question stands, no `[!decision-candidate]` stands — answered or not — no `[!todo] #review/command` carries an unfinished mini-plan, no approval reset is pending, and bootstrap has happened. The finalize verb checks the two callout conditions itself and refuses, naming each line, when a candidate or an open command is still in the body: a ticked candidate the writer never folded is that writer's unfinished round, so drop `review_approved`, phase back to `main`, `review_main_done` empty, and re-dispatch the main writer with the fold as the round's input — never delete the callout by hand, and never edit the writer's body to fold it yourself. Finalize strips the review's ephemeral markup — banner, gesture checkboxes, edit markers, system callouts, owned H1 sections — **unsets** every `review_*` key rather than writing any of them false, and stamps `review_result` with `approved` or `approved-with-concerns`, the one key it keeps. A finalized document therefore carries no `review_active` at all, not `review_active: false`; when the unset loop empties the frontmatter block entirely, finalize drops the block too. Terminal-owned sections survive the strip by design: a downstream transition reads the operator's choices there and is responsible for removing the section once its own work is done. Validation-owned sections survive only under approve-with-concerns — and every preserved section keeps its `#expert/<flat>/<id>` ownership tag: a concern the operator chose not to resolve stays attributed to the expert that raised it.
...
**The stuck marker — an `error` outcome and a died job are the coordinator's to decide.** The postman sweeps `edited` and `empty` alike, because those are exactly the two outcomes `collect-job` consumes: both clear `active_job`, raise the `job-done` wake, and dispatch the coordinator. An `empty` wake is an ordinary job-done wake whose landing happens to change no bytes — the writer had nothing to say this round — and it is read by the Chapter 3 ladder like any other. A DONE job reporting `error`, and a job that died, are left exactly as found: not landed, not consumed, `active_job` not cleared, no pending wake raised — and therefore **no job-done wake is generated at all** (an `error` belongs to the pump's retry ladder, which owns it until it is spent). A wake that reads an `active_job` still standing in `parse-note`'s `job_markers` block while its job sits in a terminal state has inherited that decision: re-dispatch the same turn, escalate to the operator, or drop the turn and clear the marker. Deciding is not optional — a marker that never clears is a document that never wakes on job-done again. In the retired ladder a dead job silently counted as having spoken and the barrier moved on regardless; that silence is exactly what Chapter 6's escalation replaces.
...
**Conditional chapter — read `${CLAUDE_PLUGIN_ROOT}/references/lazy-review.commands-playbook.md` when `parse-note`'s report shows a non-empty `> [!todo] #review/command` callout.** `> [!todo] #review/command` is the operator's free-text channel into the loop — deliberately not a checkbox (`- [ ]` is reserved for an ask-the-operator gesture) and deliberately outside the banner, which the system repaints over. A pending command runs on EVERY wake, first, whatever trigger woke you, and it runs on an otherwise stuck document; it is also the operator's manual wake. The extracted chapter carries the mini-plan unfolding with its `✓`/`→`/`·` marks, mid-command failure locking, and completion into `# History`. On a wake with an empty callout this chapter contributes nothing beyond this summary.
...
`# History` is one line per **landed round** of the document — the opening round included — composed by the coordinator and handed to `commit-doc --history "<line>"` in the same wake that lands it; the verb places it under today's `#### <date>` day group through the core's `history-append`, so the coordinator never edits the section itself. "Awaiting operator" is not an exemption: a round whose landing leaves no line has not been journaled. There is no historian job and no historian gate on finalize — the entry cannot land in an already-closed document because it is written before the transition that closes it.

**The section is bootstrapped, tagged, and terminal.** The entry verb (`start` / `submit`) creates the empty section at the end of the body with `#protected/review/history` as its first content line, followed by an explainer line written as an HTML comment (`<!-- ... -->`, in the vault's language) — the section's self-description for the operator, owned by the entry verb, never one of the coordinator's lines. The comment form is the current one: an asterisk-italic line (`*...*`) is what earlier seeders wrote, and the entry verb replaces it with the comment on any later entry, so a document that has been through a re-open carries the comment and nothing else. That tag is the section's identity (recognition is tag-based, never title-based) and marks it persistent under the cross-plugin protected-section contract — it survives every pass, finalize included. Below the explainer the lines sit in day groups, `#### <date>` then one `- ` bullet per line, oldest day first — the shape `lazy-core.markdown-style.md` § History sections fixes for every plugin. The verb owns that shape: the coordinator never creates a second `# History`, never removes the tag or the explainer, never writes a day heading or a bullet by hand, and the section stays the document's last H1: bottom-positioned validation sections insert before it.
...
**Shape.** One sentence, no date, no bullet — the verb adds the day group and the bullet. When several unrelated things changed in the same approved state, name the most significant and mention the rest by count rather than listing them.
...
- **`lazy-core.markdown-style.md`** (lazycortex-core) — the shapes of the review's callouts: `[!question] #review/question` with its `- [ ]` options, `[!todo] #review/command` with its mini-plan marks, and the banner's `#review/<state>` forms. The `#review/*` set is **closed**; a callout outside it is an audit failure, not a variation.
