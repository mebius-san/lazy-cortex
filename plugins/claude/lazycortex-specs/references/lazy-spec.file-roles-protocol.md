...
|------|---------|--------------------------|------------------------------|-------------------|
| `vision` | The goals of an asset or a level; sections per its template. Goals live ONLY here; the sibling design opens with a reference to it. Three levels: content-root (`vision.md`, the mandatory vault spec, typed `system-vision`), product root (`<spec_path>/vision.md`, seeded at product creation, typed `system-vision`), and asset folders per the type's `vision` contract (typed `vision`) — never on a bug | **No** | No | **Yes** |
| `use-cases` | Opt-in requirements scenarios written before the behavior settles — WHO needs what of the system, in the actor's own language, with no system internals; sections per its template. Three levels: content-root (`use-cases.md` — the system's actors and cross-product scenarios, type `use-cases`), product root (`<spec_path>/use-cases.md` — the product's actors and cross-feature scenarios that the assets' own use cases refine, type `system-use-cases`), and feature/change asset folders (type `use-cases`); never on a bug | **No** | No | **Yes** |
| `design` | WHAT the system does (feature/change/operator-defined-asset doc); sections per its template | **No** | No | **Yes** |
| `architecture` | Opt-in code-structure design — the SHAPE of the code, not the behavior; sections per its template. Feature/change asset-level only, never on a bug. Mandatory once the coordinator judges the asset code-bearing (`lazy-spec.coordination-playbook.md` Chapter 3/8), same as `design` is mandatory once the category needs it | **No** | No | **Yes** |
| `ui-design` | Opt-in interface design, sections per its template, with the HTML mockups filed flat beside it as attachments. On a feature or change asset (type `ui-design`), or loose at a product root as the product's shared look (type `system-ui-design`); never on a bug, never at the content-root | **No** | No | **Yes** |
| `tech` | Technical specification, sections per its template. System-level only (`tech.md` at a product root, or loose at the content-root as the project-wide spec; type `system-tech`) — no per-asset `tech.md` | **Yes** | **Yes** | **Yes** |
| `code-plan` | Opt-in development plan, sections per its template. Populated once `design.md` is approved | **Yes** | **Yes** | **Yes** |
...
| `bug` | Report doc for a bug, sections per its template. No companion `design` or `tech` — a bug folder ships only `bug.md` plus whichever quartet members are opt-in-authored | **Yes** (only in the `## Related code / logs` section) | No | **Yes** |
| `research` | The research report — the `research` tool's deliverable on a research asset, sections per its template. The only authored doc where arbitrary external references are allowed — in `## Sources` and beside the finding that cites them; every other row's URLs are source-code (forge) links, not external references. Its questions live in the sibling `design.md` typed `research-design` | **Yes** (only in `## Sources` and inline beside a finding) | No | **Yes** |
...
| `product` | Product-root LEVEL note (`<spec_path>/<leaf>.md`): the five level gates plus `spec_halted`, and the coordinator's own body sections. Owns the five system documents loose at that product root. See [status-note](./lazy-spec.status-note-protocol.md) Part 4b | **No** | No | No — carries **level gates** |
| `catalog` | Catalog-root LEVEL note (`<content-root>/<basename>.md`): the same five level gates and the same sections, owning the three system documents (`vision.md` / `design.md` / `tech.md`) loose at the content root — the content root carries no `ui-design.md`, and its opt-in `use-cases.md` (type `use-cases`) is no ladder document — plus the split into products. Exactly one per vault. See [status-note](./lazy-spec.status-note-protocol.md) Part 4b | **No** | No | No — carries **level gates** |
...
| `use-cases` | `use-cases.md` | content-root (type `use-cases`), product root `<spec_path>/` (product-level use cases, type `system-use-cases`), the feature asset's own folder, or `<spec_path>/changes/<slug>/` (type `use-cases`) — opt-in requirements-scenario doc at every level, never on a bug |
...
| `spec_stage` | stage-bearing authored docs (`vision`, `system-vision`, `use-cases`, `system-use-cases`, `design`, `system-design`, `architecture`, `ui-design`, `system-ui-design`, `code-plan`, `test-plan`, `bug`, `system-tech`, `research-design`, `research-report`) | per-file lifecycle stage, one of `empty | draft | approved | rejected | cancelled | deferred`; mirrored to a `spec/<stage>` tag. See [lifecycle](./lazy-spec.lifecycle-protocol.md). `code-report` / `test-report` / `decisions` carry NO `spec_stage` |
...
| `spec_source_requests` | every stage-bearing authored doc (`vision`, `system-vision`, `use-cases`, `system-use-cases`, `design`, `system-design`, `architecture`, `ui-design`, `system-ui-design`, `code-plan`, `test-plan`, `bug`, `system-tech`, `research-design`, `research-report`) AND `status` folder-notes | per-doc subset on authored docs / asset-wide union on the folder-note. List of path-qualified wikilinks to request files that contributed (`[]` when created directly). Forward-only; the reverse link lives in the request body. The body's `# Sources` section is a projection of this key — see [sources](./lazy-spec.sources-protocol.md) Part 1. `code-report` / `test-report` carry neither key — they are execution journals, not sourced deliverables |
