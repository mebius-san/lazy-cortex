...
- **`attach` also accepts a level document.** Besides an asset's folder-note, an `attach` line may name a system document loose at a product root or at the content root (`vision.md` / `design.md` / `tech.md`, plus `ui-design.md` at a product root). Apply stamps `spec_source_requests` onto that document directly and reopens its review — a level note carries no `## Source requests` section, so there is no intermediate stamp on the note and no status folder-note is looked for.
