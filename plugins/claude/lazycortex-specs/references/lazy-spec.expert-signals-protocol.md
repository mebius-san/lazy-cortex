...
- `blocked: <subtype> <detail>` — detail on an `outcome: error` response, never an outcome of its own: the pump's success gate reads only `outcome`, so a response carrying `blocked` without `outcome: error` is malformed, and a consumer must not branch on `blocked` in any other company. It is the planner's last resort when it cannot produce a plan; two subtypes:
...
- **Restate or quote another spec document.** Every document kind owns its own content — the vision its goals, the design its behaviour, the tech its technologies, the architecture the shape of the code — and a document says only what is its own. Material another document already holds is referenced by a wikilink to that document, never copied, paraphrased, or quoted into this one; a document that needs the other's content read side by side links to it. The same holds within one kind across levels, where `lazy-spec.doc-height-protocol.md` fixes the direction of the link.
