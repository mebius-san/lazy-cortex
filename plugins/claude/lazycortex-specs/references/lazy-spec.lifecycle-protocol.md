...
A LEVEL note — the catalog root's (`spec_role: catalog`) or a product root's (`spec_role: product`), see [status-note](./lazy-spec.status-note-protocol.md) Part 4b — carries its own four booleans instead of the five above:
...
- **A gate may be undeclared.** An operator may declare in a level note's `# Coordinator rules` that this level needs no `design`, no `ui-design` or no `tech` document at all (a `tech.md` at stage `cancelled` says the same after the fact), and `ui-design.md` is a product-root document only — the catalog root never owes one, so `spec_ui_design_done` there reads closed on absence like any optional document. An undeclared gate is not a gap and never a finding — the level playbook owns which of the four this level actually has.
