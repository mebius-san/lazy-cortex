...
description: Level playbook for the catalog root and every product root — the ladder of system documents, the five derived level gates, the launch checkboxes that write them, the candidate requests a re-approval raises, and the routing decisions the catalog root owns.
---
...
- **`use-cases.md`** (`system-use-cases`) — optional, **product roots only**. What the product's users need of it as a whole, in their own language: its actors and the scenarios that cross its assets, never a system internal — the document each asset's own `use-cases.md` refines, the way an asset design refines the product design. It sits between the vision and the design in the reading chain (§ 3). The catalog root never carries one: no `Write use-cases` row hangs there and its gate reads closed on absence like any optional document. A `use-cases.md` the operator wrote at the content root is the project-level document typed `use-cases`, not a document of this ladder: the catalog root promotes nothing on it and its gate stays closed.
...
## 2. The gates are derived, all five of them

| Gate | Closes when |
|---|---|
| `spec_vision_done` | `vision.md` exists and its stage is `approved` |
| `spec_use_cases_done` | `use-cases.md` is ABSENT, or exists at stage `approved` |
...
**A parked document holds its gate open.** `deferred` is neither absent nor approved, so a `use-cases.md` / `design.md` / `ui-design.md` / `tech.md` sitting parked leaves its gate false exactly as a draft does — the level owes the document, the operator has simply set it aside. A parked `vision.md` leaves `spec_vision_done` false the same way.

**Only `vision.md` is owed.** The other four documents are optional, and an optional document a level never wrote is not an open obligation — it is a level that does not need one. So an absent `use-cases.md` / `design.md` / `ui-design.md` / `tech.md` closes its gate exactly as an approved one does, and only the middle state — the document exists and its stage is anything other than `approved` — holds the gate open. A level with nothing but an approved vision therefore reads all five gates closed, which is the honest reading of it: nothing is owed. `vision.md` is the one exception, mandatory at both levels, so its absence never closes anything.
...
**Downward reconciliation.** A gate already closed goes stale the moment its document appears at a stage other than `approved` — a freshly seeded document, a reopened review, a rejected re-approval. Turn it back off with `flip-gate --off --auto` and re-run the upward checks against the fresh state in the same pass, so a dependent checkbox disappears that same cycle rather than a tick later. Seeding one of the four optional documents therefore OPENS its gate, and the gate closes again when the review approves it — the gate tracks the document's own state, never the fact that work once started.

**Halt.** `spec_halted: true` takes every checkbox row down and silences every enactment, but never the five gate booleans themselves and never a `# Coordinator commands` block.
...
|---|---|---|
| `Write use-cases` | `spec_vision_done` closed AND the level is a product root AND `use-cases.md` doesn't exist | seed `use-cases.md:system-use-cases`, then seed-then-start below |
...
All four `Write` rows hang together on a product root, the moment `spec_vision_done` closes; the catalog root hangs `Write design` and `Write tech` only. **Which document the operator starts first is their choice, not this playbook's** — nothing here says design before tech, and no row waits on another row's document. Two ordering constraints hold in the ladder, and neither takes a row down: the vision gates every `Write` row, and on a product root the use cases hold the design's review.

**The use cases hold the design's review.** The reading chain at this level is vision → use-cases → design, exactly as on a feature asset (`lazy-spec.feature-playbook.md`, "Use-cases and ui-design"): when a product's `use-cases.md` exists, `design.md`'s review is not continued until `use-cases.md` reaches `approved` or `cancelled`, and the design's writer reads it once it is approved (a cancelled `use-cases.md` releases the hold with nothing to read). An absent `use-cases.md` blocks nothing. The hold never takes `Write design` down and never stops the seed: a tick on that row while `use-cases.md` is still unsettled seeds `design.md` as usual, and the `lazy-review.start` of seed-then-start waits for the wake on which `use-cases.md` reaches `approved` or `cancelled`.
...
**Two ways a row does not hang at all.** The operator may declare in the level note's `# Coordinator rules` that this level needs no such document — "no tech document at this level", "no ui-design here", "no use-cases here" — and the row is then not hung, ever. Independently, a `tech.md` that exists at stage `cancelled` means the same thing after the fact: the row does not come back, and `spec_tech_done` — which a cancelled document leaves neither absent nor approved — stays out of the level's declared gates rather than reading open forever. A `use-cases.md` at stage `cancelled` reads the same way for `Write use-cases` and `spec_use_cases_done`. `lazy-spec.set-stage` permits `system-tech` and `system-use-cases` to be cancelled; `system-vision` and `system-design` are never cancelled.
