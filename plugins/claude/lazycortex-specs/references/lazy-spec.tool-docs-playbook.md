...
The source of this tool's report is `design.md`. When the design is re-approved after `docs-report.md` was accepted, the report keeps its stage-less journal form — nothing moves on it — while `spec_develop_done` turns off and `Start implementation (docs)` hangs again; the tick continues the same expert against the re-approved design, who appends to the journal and re-submits it.
