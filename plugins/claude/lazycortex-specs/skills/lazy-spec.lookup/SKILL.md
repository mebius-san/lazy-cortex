...
1. Resolve the anchor through the plugin CLI — `by-key` for a product key, `by-path` for a vault-relative path:
   ...
   **Resolve `<core-cli>` once, before that call.** It is the core plugin's `bin/lazycortex-core` file: when this repo authors the plugin itself (`plugins/claude/lazycortex-core/.claude-plugin/plugin.json` exists) that is `<repo-root>/plugins/claude/lazycortex-core/bin/lazycortex-core`; otherwise `Read` `$HOME/.claude/plugins/installed_plugins.json` and take `<installPath>/bin/lazycortex-core` from the last `lazycortex-core@lazycortex` record. Hold the absolute path and run the verb through the interpreter — never as a bare command: the file carries no exec bit and no plugin `bin/` is on `PATH`.
