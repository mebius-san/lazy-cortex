...
repeat up to 20 times (ponytail: fixed ceiling — a real ladder settles in a handful of
  ...
  2. Bash("${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-specs" coordinator-dispatch '{"path": "<repo-relative asset-note-path>"}')
  ...
  4. Otherwise (`action == "dispatched"`) a fresh job was queued — drain it:
       run Bash("${LAZYCORTEX_PYTHON:-python3}" <core-cli> expert-pump-once) again as long as it reports processed > 0.
...
AskUserQuestion: header "Next gesture", question "What should happen next on <product>/<path>?", options below with descriptions.
```
...
- **`agent not found in dev plugin: <ref> → .../cache/<plugin>/<version>/agents/<name>.md`** — the resolved `LAZYCORTEX_PLUGIN_DIRS` entry points at a cached plugin version that predates the agent this job needs (common right after a dev-vault-only change that hasn't been published yet) → when this checkout IS the dev vault that ships the agent, confirm Phase 1's derivation put the `plugins/claude/<plugin>/` source dir ahead of the cache entry for the same plugin; in a consumer checkout, update the plugin so its cache carries the agent.
