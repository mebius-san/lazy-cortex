...
surface_sha: b6bb704bac932d9a9ce2910bb23e8bc3dd0bb3b0fcd096c52ee3260da78c15e2
---
# Requests
...
**An already-launched target gets a change instead of a direct attach.** Before proposing a plain attach, the routing coordinator checks whether the target asset has already started implementation — a `spec_develop_done` gate already true, an active job whose checkbox names an implementation-side step, or an existing implementation report. Any one of those being true means the routing coordinator does not offer a direct attach at all: it proposes a change-spawn instead, naming the launched asset through the new change's `targets=` field, so the request travels through the change's own design-and-cascade path instead of editing the launched asset in place. An asset carrying none of those signals stays an ordinary attach candidate.
