...
surface_sha: 94e95a71d641e2692e74f8de0b049854c612f50de8bbb48d93667d2481bd3825
---
...
- A `tech.md` — narrow, with forge-correct links to the manifests — naming the stack and the platforms.
...
- Optionally a product-level `use-cases.md` — typed `system-use-cases`, a distinct document type from an asset's own `use-cases.md` — actors and cross-feature scenarios, if you opted in when `lazy-spec.create-from-code` asked.
...
- At least one expert registered in `lazy.settings.json[experts]` for each review role you plan to assign — use-case-writer, designer, system-designer, architect, ui-designer, planner, developer, and tester (plus data-writer and researcher if your product needs them, and editor if you want a second pass on prose before you review — it is the one role you can leave unassigned) — unless this is not your first product in the vault, in which case you can ride the shared expert set an earlier product already set up. If you have not set up experts yet, run `/lazy-spec.install` — it offers to configure them — or run `lazycortex-experts` to compose the personas first.
...
When the wizard finishes, the skill writes the product record into settings, creates the product folder with its folder-note — now the level note the `spec.catalog-coordinator` owns, carrying `spec_role: product`, the five level gates (all starting `false`), and the coordinator's own sections (`# Summary`, `# Gates`, `# Attachments`, `# Status brief`, `# Coordinator rules`, `# Coordinator commands`, `# History`) alongside its précis and stats markers — plus the shared vault-root request inbox, and generates the built-in review classes — one per document type marked for review (use-cases, system-use-cases, design, system-vision, system-design, system-tech, system-ui-design, code-plan, test-plan, bug, plus the implementation and testing report docs) — reusing the vault's shared set when your expert choices match it. It then runs `/lazy-spec.audit` automatically and reports any issues.
...
Once `design.md` is written, the skill asks one question: also author the product-level `use-cases.md` — typed `system-use-cases`, distinct from an asset's own `use-cases.md` document type — (actors and cross-feature scenarios) from the same code survey? This is opt-in — decline and the doc is simply never created, with no gap to fix later.

**`tech.md`** is narrow: the stack the product is built with and the platforms it runs on, each entry with its version and a forge-correct link to the manifest or configuration that proves it. It never describes how the code is arranged — that is the assets' own `architecture.md` — and when your project has a content-root `tech.md`, the product's copy names only what differs from it.
