...
**Running the daemon on only some machines for a file-synced checkout** — if this checkout lives on a path synced across machines (Dropbox, iCloud, Syncthing), set `daemon.run_here` to a list of hostnames (e.g. `["nexus"]`) instead of `true`/`false` — the sync would otherwise copy a bare `true` to every machine holding that path and start a daemon on all of them. Re-run `/lazy-core.install` on a host that isn't in the list and it tears down any supervisor unit that checkout already has on this host, instead of leaving a stray one running.
