...
**Skipping the daemon while keeping experts** — during install, the two daemon gates are asked once each. Answering "No" to `daemon.enabled` permanently skips all daemon-only setup (routines, supervisor, sandbox), but expert registration and the `.memory/` directory still bootstrap — experts work in interactive dispatch too, not only through the daemon. Answering "No" to `daemon.run_here` skips the supervisor on this machine while leaving the project's daemon config intact for other checkouts.
...
**Re-running after `git clone`** — rules, templates, `lazy.settings.json`, and `lazy.runtime.sh` are committed into the repo, but daemon supervisor units (launchd plist / systemd service) and `daemon.run_here` are per-machine and not committed. Re-run `/lazy-core.install` after cloning to install the supervisor for the current machine. It reads both daemon gates silently when they're already on record — if `daemon.run_here` is unset, it asks once for this machine.
