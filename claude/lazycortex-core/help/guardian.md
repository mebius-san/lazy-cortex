...
**Subtree-only publish.** Pass a glob to `/lazy-repo.mark-public` — for example `claude/**` — and it runs in subtree-public mode: it audits only those paths, writes `public_scopes` into `.guard-public.json`, and skips the GitHub visibility step. The pre-commit hook then limits its checks to files under those globs on every future commit.
