...
**Resolve `<wiki-cli>` once, before the first call.** It is the wiki plugin's `bin/lazycortex-wiki` file: when this repo authors the plugin itself (`claude/lazycortex-wiki/.claude-plugin/plugin.json` exists) that is `<repo-root>/claude/lazycortex-wiki/bin/lazycortex-wiki`; otherwise `Read` `$HOME/.claude/plugins/installed_plugins.json` and take `<installPath>/bin/lazycortex-wiki` from the last `lazycortex-wiki@lazycortex` record. Hold the absolute path and run every verb as `Bash("${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> <verb> …)` — never as a bare command: the file carries no exec bit and no plugin `bin/` is on `PATH`.
