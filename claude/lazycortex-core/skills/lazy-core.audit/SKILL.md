...
- References (`.claude/references/*.md`, `$HOME/.claude/references/*.md`, `claude/*/references/*.md`) — `wc -c` via Bash.
...
**Skill-writing compliance** — see `lazy-core.skill-writing`. File set: `.claude/skills/*/SKILL.md`, `claude/*/skills/*/SKILL.md` (commands exempt from the preamble check). Eight checks:
...
8. **Question context** — see `lazy-core.skill-writing § 11`. Widen to `.claude/commands/*.md` and `claude/*/commands/*.md`. For every body site that instructs the agent to raise an `AskUserQuestion` (skip `allowed-tools:` lists, negations such as "do NOT open an AskUserQuestion", and mentions of other skills), check that a `Context (print before asking):` block precedes it, or — for a policy paragraph describing a class of question — that the paragraph names the four items (where, found, why asking, answers). Missing → `[WARN] AskUserQuestion site without a context block | <path>:<line>`.
...
**Reference-writing size budget** — see `lazy-core.reference-writing § 4`. File set: the same one already measured under **On-demand sizing** above (`.claude/references/*.md`, `$HOME/.claude/references/*.md`, `claude/*/references/*.md`); reuse those `wc -c` byte counts rather than re-measuring.
