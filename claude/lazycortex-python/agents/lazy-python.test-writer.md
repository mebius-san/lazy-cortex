...
- Class docstring: starts with `"Test unit for "`. Summary line only, no sections, in the multi-line quote form the format checker requires (`"""`, newline, the summary line, newline, `"""`) — a single-line `"""Test unit for X."""` is a D1 finding.
...
- Method docstring: starts with `"Test that "`. Summary line only, no sections, in the same multi-line quote form as the class docstring.
...
- A test file passes the same `pcf` format checks as production code when its directory is checked: use the import layout of the Python scaffold template (`from __future__ import annotations`, stdlib imports, the `if TYPE_CHECKING:` block, two blank lines before code), annotate every test method (`tmp_path: Path`, `-> None`), and give helpers a `Returns:` section.
