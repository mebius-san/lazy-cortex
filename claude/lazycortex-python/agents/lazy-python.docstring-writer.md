...
**Section order:** Summary, Scope, Responsibilities, Guarantees, Generation Rules, Value Ranges, Subclassing, Notes, Type Parameters, Attributes.

- **Summary:** Noun phrase. No "This class ...". No section title. No empty line before it.
- **Scope:** No title. Empty line before. Describe what the class represents and how callers use it. **Strictly prohibited in Scope:** input args, base class names, inheritance details, internal data structures, implementation mechanisms (storage, serialization, persistence), API/protocol wiring details, and generic architecture jargon that doesn't help a caller understand the class. If you can't write a Scope that adds genuine caller-visible information beyond the Summary, omit it entirely. For enums, describe what the enum represents invariant to member changes. For data-set-initializer classes (those consumed by an AI generator for value generation): detailed data-set description — include generation rules, value ranges, and detailed attribute descriptions. You may add new sections to existing data-set-initializer docstrings but must not alter existing Scope, Value Ranges, or Attributes content.
- **Generation Rules:** Use only for classes whose data is generated by an AI entity generator (data-set-initializer subclasses and other generator-consumed prototypes — your project's overlay declares the concrete base classes). Content must be derived from code — every rule must reference a `DOC(...)` comment or specific code mechanic. Never include balance requirements, profile shape guidance, or subjective thematic rules not enforced by code. Discover existing `DOC(...)` rules by grepping the codebase first. Sub-category headers end with a colon, followed by indented bullets. Include source references as `# REF:` lines (stripped during generation), e.g. `# REF: see DOC(<group>) in <module>.py`. 2-space indent under the title. Empty line before a section.
- **Value Ranges:** Use only for data-set-initializer classes with bounded numeric domains. Define canonical ranges and their interpretation for the whole data set. Do not duplicate per-field semantics from Attributes. 2-space indent under the title. Empty line before a section.
...
- **Attributes:** Mandatory when the class has any public attributes to show. Omit only when there are zero qualifying attributes.
  - **Include:** Public instance fields assigned in `__init__` as `self.name` without a leading underscore. Public class variables without a leading underscore. Private fields if listed in `_field_filters`. Properties if listed in `_field_filters`. All enum members except `INVALID`.
  - **Exclude:** All private (`_`-prefix) fields not in `_field_filters`. All `@property` methods not in `_field_filters`. Type annotations. The `INVALID` enum member.
  ...
  - **Style:** `name: description` format. No bullets, dashes, or prefixes. 2-space indent. Noun phrase per item, period at the end. Empty line before a section. Separate public and private groups with an empty line if both exist. For data-set-initializer classes, use full detailed descriptions for AI-driven value generation.
...
- No private attributes in the Attributes section.
- No `@property` methods in an Attributes section.
...
- No custom sections outside the defined set (Summary, Scope, Responsibilities, Guarantees, Generation Rules, Value Ranges, Subclassing, Notes, Type Parameters, Attributes, Args, Returns, Yields, Raises, Overriding).
...
8. **No private attrs in `Attributes:`.** Every label in the `Attributes:` section either has no leading `_` or is listed in the class's `_field_filters` dict. No exceptions.
...
Trips: #8 (no `_field_filters` declaration on the class).

**Good**: omit the section entirely (zero qualifying public attributes), or if the class declares `_field_filters` listing these names, keep them.
