...
In refile mode the edit is one header line per refiled block — the new group, the block's indentation and comment prefix preserved. Nothing else in the file is touched: not the title line, not the body, not a neighbouring block.
