...
4. **`# opt:` / `# limit:` semantics.** `pcf` proves only that the clause after the colon is non-empty; whether it says the right thing is yours. `opt:` annotates a choice made for performance — on code with no performance motive it is a finding. `limit:` annotates a deliberate simplification that is correct at the current scale: the clause must name the ceiling and the upgrade path past it, and the code must be finished as written — a `limit:` on unfinished work is a `TODO:` mislabelled, and a `limit:` whose clause names no condition under which the code stops being adequate is a finding.
