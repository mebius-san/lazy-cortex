...
4. **`# opt:` / `# limit:` / `# Decision:` semantics.** `pcf` proves only that the clause after the colon is non-empty; whether it says the right thing is yours. `opt:` annotates a choice made for performance, and its clause names the assumption outside the marked lines that keeps it correct — on code with no performance motive it is a finding, and so is a clause stating the speed win instead of the assumption, or a cache, memo, or precomputed table carrying no `opt:` at all (`pcf` catches only the `functools` decorators; hand-rolled ones are yours). `limit:` annotates a deliberate simplification that is correct at the current scale: the clause must name the ceiling and the upgrade path past it, and the code must be finished as written — a `limit:` on unfinished work is a `TODO:` mislabelled, and a `limit:` whose clause names no condition under which the code stops being adequate is a finding. `Decision:` records a real design fork: judge it by the canon's three tests (a fork existed, revisiting is costly, the why is not recoverable from the code) — a marker on the only viable move, on a repo convention, or with a thesis whose why is obvious from the code is a finding. Docstring prose that retells why the code is written one way and not another is a candidate for extraction into a `Decision:` marker — flag it.
...
6. **Useless intermediate variables.** A local used exactly once, or aliasing a trivial accessor, is a finding unless it is an expensive result reused, a loop-invariant hoist, a named complex expression, or an access chain two or more objects deep read more than once.
7. **Docstring substance.** Docstrings describe external behaviour; they do not narrate implementation, private internals, or call sequences. Documented `Raises` / `Returns` / `Args` match the code's actual contract — a docstring that describes behaviour the code does not have is a finding against the code or the docstring, and you say which.
8. **Structural placement.** Entity placement follows the overlay's module-structure rules (where enums live, what a package exports, what may sit at module scope). Report a violation only against a clause a layer actually states.
...
13. **Mirrored contracts.** A `Contract:` block repeated on both an interface (or abstract base) declaration and an implementation of the same method is a `WARN`: the guarantee belongs on the interface declaration, so the finding names the implementation's copy as the one to drop and expects the implementation's `Guarantees` section to keep tracing to the interface. A block that states an extra guarantee only that implementation adds is not a mirror and is not a finding.
14. **Optimizations against the change.** When the diff touches a `Contract:` block, a `Domain(…):` block, or the body of a function in a file, re-read every `opt:` in that file — changed or not — and check its named assumption against the change. An assumption the change broke while the optimized code stayed as it was is a `FAIL`; the fix goes to the optimization or its marker, never to the assumption text alone.
15. **Knowledge-derived tests.** A changed or added `Contract:`, `Domain(…):`, or `opt:` block whose derived test did not change with it is a finding: the canon takes at least one test from each (`lazy-python.testing-guidelines.md` *Knowledge-derived tests*). Judge only the touched regions; a pre-existing block with no test is not this review's debt. A `Domain(unfiled):` block is judged like any other here — parking its group does not park its test.
