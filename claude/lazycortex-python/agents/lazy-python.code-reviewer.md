...
4. **`# opt:` / `# limit:` / `# Decision:` semantics.** `pcf` proves only that the clause after the colon is non-empty; whether it says the right thing is yours. `opt:` annotates a choice made for performance — on code with no performance motive it is a finding. `limit:` annotates a deliberate simplification that is correct at the current scale: the clause must name the ceiling and the upgrade path past it, and the code must be finished as written — a `limit:` on unfinished work is a `TODO:` mislabelled, and a `limit:` whose clause names no condition under which the code stops being adequate is a finding. `Decision:` records a real design fork: judge it by the canon's three tests (a fork existed, revisiting is costly, the why is not recoverable from the code) — a marker on the only viable move, on a repo convention, or with a thesis whose why is obvious from the code is a finding. Docstring prose that retells why the code is written one way and not another is a candidate for extraction into a `Decision:` marker — flag it.
