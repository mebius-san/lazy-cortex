...
class DocstringAnalyzer(ast.NodeVisitor):
  ...
  def _gen_rules_line_indices(self, docstring: str) -> set[int]:
    ...
    This section is owned by the `acme-gen.check-rules` skill; D-checks must
  ...
  def _check_banned_phrases(self,
    ...
    skipped -- it is owned by the `acme-gen.check-rules` skill.
    ...
    for phrase in self.banned_docstring_phrases:
      ...
      if not phrase:
        continue
      for idx, line in enumerate(lines):
        # guard: Generation Rules section is owned by acme-gen.check-rules
        if idx in skip_idx:
          continue
        if phrase in line:
          self.issues.append((
  ...
  def _check_marker_in_docstring(self,
    ...
    `acme-gen.generation-rules.md` rule mandates `# REF:` lines there as source
