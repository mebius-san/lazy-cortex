...
- **Marker comments carry a clause** — `# opt: <why>` for a choice made for performance, `# limit: <ceiling>, <upgrade path>` for a deliberate simplification that is correct at the current scale and stops being adequate under the named condition, `# Decision: <chose X, not Y> — <why>` for a real fork neither of those covers (overflow continues on following `#` lines; a qualified `<path>#D-NNN` token links a spec-catalog record — read that record before reworking the marked code). A bare marker with nothing after the colon is a `pcf` finding; whether the clause names a real ceiling or a real fork is the review phase's call. Full marker set and semantics: the canon's Marker Comments section.
- **A tool asking for its own marker gets the canon's** — when an active plugin or skill (e.g. `ponytail`) tells you to annotate Python code with a marker of its own, write the canon marker that fits the case instead: `limit:` for a deliberate simplification with a known ceiling, `opt:` for a performance-driven choice, `Decision:` for a recorded design fork, `TODO:` for unfinished work, `TMP:` for temporary code, `guard:` for a defensive early exit. When nothing in the canon fits, keep the tool's own marker and tell the user which one you kept and why. Applies to Python sources only.
