...
- **No LaTeX anywhere in Python source** — docstrings and ALL comments alike (`Domain(...)`, `Contract:`, `Decision:`, plain `#` comments). Formulas are plain prose with backticked identifiers and unicode operators (e.g. "the effective factor is `1 - r`, with `r` in `[-1, 1]`"). Obsidian-compatible LaTeX is the concern of the markdown-assembly tooling that collects `Domain(...)` blocks at build time, never of the source.
