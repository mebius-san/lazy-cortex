...
- **Special-comment preservation** — `TODO:`, `TMP:`, `DBG:`, `REF:`, `opt:`, `guard:`, `limit:`, and `DOC(...)` comments must never be removed, reworded, or relocated when editing surrounding docstrings. Treat `TMP:`-marked code as non-existent and `TODO:`-marked code as already implemented when writing the Summary/Scope of a containing artifact; treat `limit:`-marked code as complete as written — never describe it as incomplete or in need of the named upgrade, and never carry the ceiling into the docstring.
...
- **Never silently strip or rewrite `TODO:` / `TMP:` / `DBG:` / `REF:` / `opt:` / `guard:` / `limit:` / `DOC(...)` markers** while touching surrounding docstrings — they are caller-visible invariants, not noise.
