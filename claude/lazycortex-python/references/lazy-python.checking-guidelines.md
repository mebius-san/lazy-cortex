...
The cache-marker rule is split the same way. `pcf` flags a function or property under a `functools` cache decorator (`cache`, `lru_cache`, `cached_property`) whose comment block directly above the decorator stack carries no `# opt: <assumption>`; a `# waiver: <reason>` covering the first decorator line, with the reach every waiver has, exempts it. Hand-rolled memo dicts and precomputed tables, and whether the clause names the real assumption, stay with the review phase.
