...
**`review.py`** is the guideline review phase — the seventh and final step of `chk-py all`. Unlike the other six checks, it does not run a deterministic tool against your code; deterministic checkers can only prove what an AST walk proves (import order, syntax, types), and the project's other rules — a purpose comment on every block, logical-block separation, naming semantics, your own overlay clauses — need a reader, not a parser. `chk-py review` resolves the scope (explicit paths, or your current working-tree diff plus untracked files when you pass none), collects every applicable guideline layer in precedence order — the canon reference guidelines, your `docs/guidelines/` overlay, any `.claude/rules/*.md` scoped to `.py`, and `CLAUDE.md` — and writes a manifest under `.logs/lazy-python/review/`. It then prints the manifest path and names the `lazy-python.code-reviewer` agent to dispatch against it; you (or the calling skill) run that agent, which writes a findings document, and `chk-py review --render <findings.json>` prints those findings in the same `file:line: severity: message` shape as the other checkers and exits non-zero on a `FAIL`. Because the review step never calls an agent on its own (pre-commit and CI have no LLM available), it always exits 0 as part of `all` — a pending review cannot break the gate, but it also does not clear itself; render its findings before treating the change as reviewed. A scope that has not changed since its last review reuses those findings instead of asking you to re-dispatch.
...
`chk-py all` is the canonical gate you run before committing. It executes the seven-step pipeline in order: `pcf` (import and format rules), `toi` (type-only import suggestions), `cmp` (`py_compile` syntax check), `mypy` (type checking), `rf` (ruff lint), `pylint` (semantic lint), `review` (guideline review). Each step runs to completion before the next begins; if any of the first six exits non-zero, the overall command exits non-zero. `review` is the exception — it always exits 0 within `all`, printing a manifest path and a dispatch directive instead of a pass/fail verdict, so a review that has not yet happened does not block the rest of the gate from reporting clean. Treat a printed `review: PENDING` line as work still owed: dispatch the named agent, then run `chk-py review --render <findings.json>` to get the real verdict, which does exit non-zero on a `FAIL`.
...
**Rendering a pending guideline review.** `chk-py review` on its own only writes a manifest and prints a dispatch line — it never changes exit code within `all`. Once the named `lazy-python.code-reviewer` agent has run against that manifest and written its findings document, run `chk-py review --render <findings.json>` to print the findings and get the real pass/fail exit code.
