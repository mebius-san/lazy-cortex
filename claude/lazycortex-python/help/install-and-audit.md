---
chapter_type: block
summary: Bootstrap lazycortex-python with a 10-step install wizard (incl. env_source detection) and verify with the 11-check read-only audit.
...
source_skills:
  - lazy-python.install
  - lazy-python.audit
source_sha: 09ee8f0561b8dcfdeebb74bd460aabbace7b8102
---
# Install and audit
...
**Code-reviewer model tier and expert registration.** Install seeds `lazy-python.code-reviewer` (alongside the two writer agents) into the `agent_models.lazycortex` tier group and registers it as the `python.code-reviewer` expert in `.claude/lazy.settings.json`, both silently and both additive — a value already on record is never overwritten. Neither is checked by audit; if you need to reset either, re-run `/lazy-python.install`, which re-derives them from the plugin's shipped defaults without touching anything you've customised.
...
- **agents** — the four dispatched/manual review members, including `lazy-python.code-reviewer`, whose model tier and expert registration install now seeds.
