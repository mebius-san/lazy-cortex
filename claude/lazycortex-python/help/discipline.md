...
**Skipping the guideline-review phase because the checkers are green.** `chk-py all` proves what an AST walk can prove — syntax, types, imports, style. It cannot prove a purpose comment sits on every block, that logical blocks are properly separated, or that new code matches the project overlay's naming semantics. Run `chk-py review` after the checkers and tests are clean, dispatch the `lazy-python.code-reviewer` agent it names, and render the findings with `chk-py review --render <findings.json>` before calling the change done.
