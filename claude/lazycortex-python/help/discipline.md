...
**`lazy-python.docstrings`** also loads on `**/*.py` and enforces a single hard constraint: never write docstrings manually — dispatch the `lazy-python.docstring-writer` agent instead. The rule explains why: the agent reads the full documenting canon plus the project overlay on every dispatch, and hand-writing from session memory reliably violates at least one of the eight Self-Check clauses. The rule also covers the most-forgotten inline conventions: opening and closing `"""` each on their own line, single backticks for inline code, no descriptions of internal algorithms, and preservation of `TODO:`, `TMP:`, `DBG:`, `REF:`, `opt:`, `guard:`, and `DOC(…)` markers. Formulas stay plain text everywhere in Python source — docstrings and `DOC(…)` comments alike; turning them into Obsidian-compatible LaTeX is the job of the markdown-assembly tooling, not of the source. The full canon lives in `lazy-python.documenting-guidelines.md`.
