...
**Manual runs** (no daemon needed): mirror sync is a plain CLI call from any session — `lazycortex-wiki mirror-sync <scope-id>` fetches the source, syncs the mirror, and commits; `lazycortex-wiki mirror-plan <scope-id>` prints the dry-run plan as JSON. The daemon counterpart is the `lazy-wiki.mirror-sync.<scope-id>` schedule routine registered by `/lazy-wiki.install`. `lazycortex-wiki terms-apply <document> --from "<word>" --to "<term>"` applies a terminology decision you have already made to one document — whole-word, prose only, never frontmatter, fences, inline code, link targets or the `# See also` block — and refuses on a document under an open review round or inside a mirror tree.
