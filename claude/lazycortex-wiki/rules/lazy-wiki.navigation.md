...
**Do not answer from your own reading of the sources when the question is about material a covered scope holds.** Coverage is the `paths` globs listed under `## Coverage` below. When the answer would come from files matching one of them, `/lazy-wiki.query "<question>"` runs first — before Grep, before Glob, before opening a file in the scope. Read specific files afterwards, once the wiki has named them.
...
Do **not** read `topics.md` into your own context to search it — when the `Agent` tool is available to you. `/lazy-wiki.query`'s dispatcher mode fans out to retrieval subagents whose contexts hold the topic catalog and the traversed node bodies, returning only entry points and a synthesised answer, so *your* context — the one that can keep fanning out — stays clean. When you are already standing on a node, follow its See-also glosses directly.

A one-shot context with no `Agent` tool (an expert job, a subagent dispatched without further fan-out capability) has no downstream context to protect — there is nowhere to delegate to. `/lazy-wiki.query`'s agentless mode covers exactly this case: read the scope's `topics.md` yourself, pick entry points yourself, and follow glossed links yourself.

## Coverage

Written here by `/lazy-wiki.configure` when a scope is created or edited. Until it has run, read `.claude/lazy.settings.json[wiki.scopes]` once before deciding whether a question is covered.
...
**Backlinks** — grep the node's repo-relative path across the scope. All links are local relative paths; external material a scope needs is mirrored into the vault (a scope's `mirror` block) and linked like any local node.
