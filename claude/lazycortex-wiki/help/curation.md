...
surface_sha: 7f8a59c43f13bcd381f91eab87ba888f3e96a97bd2aeaf77937aade4c093a3bd
---
# Curation
...
**Curator output is in the wrong language.** Every piece of prose a curation pass writes — a `wiki_summary`, a See-also gloss, a connector phrase, the tag-values dictionary — follows one language resolved once per run: `wiki.language` when you set it, otherwise the repo-wide `language` key, otherwise English. Set or change it with `/lazy-wiki.configure vault`. Prose that already exists in another language is never retranslated by a later relink or daemon pass — only sections a curator writes fresh pick up the resolved language, so a scope started before the language was set may carry a mix until its nodes are re-classified. The tag-values dictionary's opening heading and paragraph are the one exception to "freshly written always matches the resolved language": they come from a small set of pre-translated wording rather than being composed by the curator, so a resolved language without its own translation falls back to English for just that heading and paragraph — every gloss and summary elsewhere in the dictionary, and everywhere else in the wiki, still follows your configured language.
