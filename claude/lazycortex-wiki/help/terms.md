...
flowchart LR
  ...
  lazyWikiTerms["lazy-wiki.terms (lookup or check-candidate)"]
  dictionaryFile["Terms dictionary file"]
  returnDefinitions["Returns matching definitions"]
  documentCommit["Document commit"]
  termsScanRoutine["terms-scan routine"]
  curatorCurate["lazy-wiki.terms-curator (curate mode)"]
  curatorCommits["Curator commits dictionary"]
  doctorTermsSection["/lazy-wiki.doctor terms section"]
  curatorReport["lazy-wiki.terms-curator (report mode)"]
  doctorPresents["Doctor presents findings to operator"]
  operatorApplies["Operator applies chosen fixes"]

  writingExpert -->|calls, lookup or check-candidate mode| lazyWikiTerms
  lazyWikiTerms -->|reads| dictionaryFile
  dictionaryFile -->|returns matching definitions| returnDefinitions
  documentCommit -->|triggers| termsScanRoutine
  termsScanRoutine -->|dispatches curate mode| curatorCurate
  curatorCurate -->|reads document and dictionary headings| dictionaryFile
  dictionaryFile -->|edited and committed| curatorCommits
  doctorTermsSection -->|checks format and scope, reading| dictionaryFile
  doctorTermsSection -->|dispatches report mode| curatorReport
  curatorReport -->|reads for meaning checks, writes nothing| dictionaryFile
  curatorReport -->|returns findings| doctorPresents
  doctorPresents -->|applies chosen fixes| operatorApplies
  ...
  class doctorTermsSection entry
  class lazyWikiTerms action
  class dictionaryFile action
  class termsScanRoutine action
  class curatorCurate action
  class curatorReport action
  class doctorPresents action
  class returnDefinitions success
