...
When there's no `Agent` tool to dispatch a seeker or gatherer to, the executing context does the same two steps itself: it reads each configured scope's `topics.md` directly and picks the entry points a seeker would, then reads those nodes and follows their glossed See-also links (falling back to a grep-based backlink search when needed) the way a gatherer would — bounded to what the question needs rather than an exhaustive walk. It synthesises the same answer-plus-sources shape, in the same resolved language, and presents it the same way. Nothing about the output changes; only where the traversal happens does.

If no scope has a topic index on disk, or if no entry points are found, the skill does not compose the no-results message itself — it asks the wiki CLI for it, passing the resolved language: `"${LAZYCORTEX_PYTHON:-python3}" <path-to-wiki-cli> text query-no-results --lang <code>`. Whatever that command returns is what you see, so the wording stays centralised in one place instead of drifting between the two modes; a configured language with no phrasing of its own for this message comes back in English rather than as an empty or missing string. This applies in either mode — dispatcher and agentless both route through the same lookup.
...
- **Content from another repository.** A See-also link written as `@<repo-key>/…` is a retired cross-repo notation — the gatherer treats any link starting with `@` as dead and reports it unresolved rather than guessing at a target. To bring another repository's markdown into a scope so it participates in traversal normally, mirror it instead: `/lazy-wiki.configure mirror` clones the source repo and syncs its markdown into the scope under a local `mirror_path`, where it becomes an ordinary local node with ordinary local See-also links. `/lazy-wiki.audit` flags any surviving `@<repo-key>/…` link as `dangling-at-prefix` and tells you whether to rewrite it to the mirrored node's local path or drop it.
- **Answer language.** The skill resolves your project's configured language once, before dispatching anything, and hands that code to every seeker and gatherer it dispatches — a subagent can't read your settings for itself. The presented answer, the no-results fallback (sourced from the wiki CLI's canonical text table rather than composed by the skill), and the seeker/gatherer prose all come back in that language, with a language carrying no phrasing of its own falling back to English; paths, tags, frontmatter keys, and quoted identifiers stay verbatim regardless. Run `/lazy-wiki.configure` to change the configured language.
