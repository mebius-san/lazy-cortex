...
**Repaired by `lazycortex-wiki doctor --apply`:** `orphan-topic` and `index-desync` rebuild the topic index; `see-also-path-base` rewrites a See-also link written against a non-canonical path form; `broken-see-also` drops a dead link; `stale-gloss` refreshes an outdated gloss.
