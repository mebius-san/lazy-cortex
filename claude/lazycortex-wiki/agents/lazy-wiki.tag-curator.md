...
5. **Apply (ALWAYS, both modes), unless the map is empty.** `$WIKI_BIN retag <surface> --from <alias-map-file> --repo <repo-root>` — MUST exit 0. In tail:false, `rm` the temp after a successful apply. An empty map → skip the call and go straight to the dictionary step; the dictionary may still need the values this surface carries.
6. **Rewrite the dictionary (ALWAYS, both modes).** Re-survey the surface as it now stands: `$WIKI_BIN collect-tags <surface> --repo <repo-root>` — this is the post-retag truth, never your own expectation of it. Then `Write` the whole dictionary file at the path from step 1:
