...
5. **(tail: true only)** `"${LAZYCORTEX_PYTHON:-python3}" "$WIKI_BIN" build-index <scope-id> --repo <repo-root>` → `git add <abs-node-path> <abs-topics-md-path> && git commit -m "wiki(classify): <node-basename>"` — do **NOT** pass `--author`; the pump set `GIT_AUTHOR_NAME` / `GIT_AUTHOR_EMAIL` in env, git picks them up automatically. → `"${LAZYCORTEX_PYTHON:-python3}" "$WIKI_BIN" dispatch-link <abs-node-path> --repo <repo-root>`.
