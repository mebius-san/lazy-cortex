...
`<wiki-cli>` stands for the wiki plugin's `bin/lazycortex-wiki` file — the newest copy under `~/.claude/plugins/cache/lazycortex/lazycortex-wiki/<version>/`, or `claude/lazycortex-wiki/` in a checkout that authors the plugin. Every verb runs through the interpreter, `"${LAZYCORTEX_PYTHON:-python3}" <wiki-cli> <verb>`: the file carries no exec bit and is not on `PATH`. The command derives the `path_filter` and `group_globs` the three structure-scan routines must carry and compares them against what is registered. A non-zero exit means at least one routine watches something the map does not describe, or dispatches per file where it should dispatch per directory. Report every entry of its `drift` object as `FAIL structure-watch-filter: <routine> <key> expected <expected>, found <actual>`, and name the fix — re-run `/lazy-wiki.configure structure`, which re-registers the drifted routines.
