...
Each of the six is independently dispatchable. If you already have a well-formed brief and want formal scenarios, dispatch the use-case writer directly. If you already have a settled brief and want to jump straight to design, dispatch the designer. If behavior is already decided and only the interface is open, dispatch the UI designer directly. If behavior is decided and only the code's shape is open, dispatch the architect directly with the design spec. The five-stage sequence above is a convention, not a constraint.

## Where this fits

- Run `/lazy-core.agent-models` to adjust which model tier each of these six agents uses.
- The **aspects** block composes domain knowledge (e.g. `lazy-experts.claude-plugin-aspect`, `lazy-experts.game-dev-aspect`) into all six via your `lazy.settings.json[experts]` entry. An aspect shapes how an agent interprets, writes use cases, designs, designs UI, architects, or plans — it never changes which agent runs or what protocol it follows.
- The **composition** block shows how to wire a concrete specialist — pairing one of these six agents with one or more aspects — in `lazy.settings.json[experts]`.
