...
The aspects block is a set of pure prompt layers — each one adds a body of knowledge or behavioral discipline to whichever generic expert (`interpreter`, `designer`, `planner`, `implementer`, `debugger`, `reviewer`, `tester`, or `fiction-writer`) you pair it with. You declare the pairing in `lazy.settings.json[experts]` and the expert runtime merges the aspect bodies into the agent's system prompt at dispatch time. The result is a named specialist — for example a `claude-plugin-planner`, a `game-designer`, or a `sci-fi-writer` — without authoring a fresh agent for each domain.
...
flowchart LR
  ...
  expertsComposition{lazy.settings.json experts composition entry}
  technicalSpecialist[Named specialist - seven engineering roles plus tech-writing]
  fictionSpecialist[Named specialist - fiction-writer, sci-fi/fantasy aspects]
  ...
  disciplineAspect -->|composes onto every entry| expertsComposition
  techWritingAspect -->|composes onto technical entries only| expertsComposition
