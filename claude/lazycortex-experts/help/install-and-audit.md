---
chapter_type: block
summary: Bootstrap lazycortex-experts by seeding agent-model tiers and pre-composed expert entries for all agent × domain-aspect pairs into lazy.settings.json.
...
- You've updated the plugin and a new release shipped additional domain aspects or revised tier entries — re-running picks up everything new.
...
**Seeding composed experts.** The skill enumerates every agent (`lazy-experts.interpreter`, `lazy-experts.designer`, `lazy-experts.planner`) and every domain aspect (`claude-plugin`, `game-dev`, `dotfiles`) from the installed plugin cache, then takes their cartesian product — currently nine combinations. For each pair it writes one entry under `experts` in `lazy.settings.json`. The entry names the agent, stacks the domain aspect, and automatically adds `lazycortex-core:lazy-memory.persona-aspect` so the expert accumulates private memory across runs under `.memory/<expert-key>/`. Example entry:
...
Expert entries are only written when absent — if you've already customised an entry, the skill leaves it untouched and reports `kept-local`.
