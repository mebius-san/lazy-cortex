---
chapter_type: block
summary: Bootstrap lazycortex-experts by seeding agent-model tiers and composed expert entries for all six agent × domain-aspect pairs into lazy.settings.json.
...
**Seeding composed experts.** On a fresh project with an empty `experts` section, the skill asks which domain aspects (classes) to register. Pick the domains your project works in — you can re-run later to extend coverage. The skill then takes the cartesian product of those classes with every available role (all six agents) and writes one entry per pair. Each entry names the agent, stacks the domain-specific aspect, and automatically includes two cross-cutting aspects: `lazycortex-experts:lazy-experts.discipline-aspect` (execution discipline applied to every expert regardless of class) and `lazycortex-core:lazy-memory.persona-aspect` (so the expert accumulates private memory across runs under `.memory/<expert-key>/`). Example entry:
