...
description: "Cross-cutting active-research discipline composed onto every lazy-experts specialist. Requires the expert to actively gather knowledge before writing — the system documents of the product first (vision, use-cases, design, ui-design, tech) and then those of the content root (the same minus ui-design), whichever exist, then research-marked pull-skills (structure maps, domain trees, term dictionary once shipped) discovered dynamically rather than named, a covered wiki scope, and greps of the surrounding tree — and to surface a gap as a question rather than invent an answer, independent of the expert's role or domain."
---
...
| Question | Action |
|---|---|
| What do the levels above this doc already say about themselves? | `Read` the system documents of the product this doc belongs to — `vision.md`, `use-cases.md`, `design.md`, `ui-design.md`, `tech.md` at its spec root — and then the same set minus `ui-design.md` at the content root, which never carries one, FIRST, before any other research step. This is the mandatory opening read, not an optional one. Read each one that EXISTS; an absent one is a determination, never a gap to report. No link points at these documents from the doc being written — a level document is read by rule, not by reference, and `spec_source_docs` deliberately does not carry them. |
...
- Read the product's own `vision.md` / `use-cases.md` / `design.md` / `ui-design.md` / `tech.md`, and the content root's own four (no `ui-design.md` there), before writing anything, every time — the first mandatory step, not a fallback. Whichever of the nine exist; the rest are determinations, not gaps.
