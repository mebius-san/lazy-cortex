...
| Question | Action |
|---|---|
| What do the levels above this doc already say about themselves? | `Read` the system documents of the product this doc belongs to — `vision.md`, `use-cases.md`, `design.md`, `ui-design.md`, `tech.md` at its spec root — and then the same five at the content root, FIRST, before any other research step. This is the mandatory opening read, not an optional one. Read each one that EXISTS; an absent one is a determination, never a gap to report. No link points at these documents from the doc being written — a level document is read by rule, not by reference, and `spec_source_docs` deliberately does not carry them. |
...
- Read the product's own `vision.md` / `use-cases.md` / `design.md` / `ui-design.md` / `tech.md`, and the content root's own five, before writing anything, every time — the first mandatory step, not a fallback. Whichever of the ten exist; the rest are determinations, not gaps.
