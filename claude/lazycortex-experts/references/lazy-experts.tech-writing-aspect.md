...
- **The `## Terms` section is the document's glossary.** When the template carries a `## Terms` section, every term the document uses stands there as one line, `**term** — definition`, in alphabetical order: a term the repository's dictionary already defines carries the dictionary's definition verbatim, and a term this document introduces is defined there before the body uses it. The section is never left empty in a document handed back, never lists a word a general reader already knows, and is the first thing the dictionary's curator reads when it takes new terms from the finished document.
...
- Fill the `## Terms` section when the template carries one: one alphabetical line per term the document uses, the dictionary's definition verbatim for a dictionary term, your own definition for a term the document introduces; never hand it back empty.
