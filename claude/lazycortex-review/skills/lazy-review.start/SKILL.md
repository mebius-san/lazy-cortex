...
2. **Apply + commit** — `python3 "${CLAUDE_PLUGIN_ROOT}/bin/start.py" <file> [--expert <name>]`. The bin script does the frontmatter edit + banner insertion + `git add` + `git commit` in one subprocess, leaving the working tree clean.
