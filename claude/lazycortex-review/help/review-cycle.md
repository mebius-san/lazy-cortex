...
- **Change edit-marker style** — `/lazy-review.start` and `/lazy-review.submit` pin the current `lazycortex-review.edit_marker_style` setting into the document's own `review_marker_style` frontmatter the moment a review opens, and `/lazy-review.finalize` reads that pin rather than live settings — so changing the setting mid-cycle never disrupts a review already in flight. Run `/lazy-review.configure` to change the style; it takes effect the next time a document opts into review, not on one already active.
