...
class TickInputs:
  """
  Snapshot of a document's review state for one dispatcher tick.

  Attributes:
    parse_failed: Whether the document's frontmatter or body failed to parse this tick.
    repair_attempts_remaining: Number of automated repair attempts still available before the
      document is skipped with a parse error.
    approved: Whether frontmatter mirrors the operator's approval of the whole document.
    main_chain_pending: Main-chain experts still queued to run before the pre-approve phase
      completes.
    section_writer_pending: Section writers still queued to run before the pre-approve phase
      completes.
    current_banner: Banner state currently rendered in the document, or `None` if no banner is
      present.
    desired_banner: Banner state that should be rendered for this tick.
    review_active: Whether the file is currently opted in to the review loop.
    approve_checkbox_ticked: Whether the operator ticked "approve the whole document" on the
      Ready banner while frontmatter has not yet mirrored the approval.
    post_approve_section_writer_pending: Post-approve section writers (final-as-section
      pattern) still queued to run.
    any_post_approve_section_non_empty: Whether at least one post-approve section still holds
      non-empty content, requiring a fresh main-writer round before finalize.
    validation_round: Number of times a post-approve validation writer has emitted a
      non-empty owned section over the document's lifetime.
    approve_with_concerns_ticked: Whether the operator ticked "approve with concerns" on the
      concerns-decision banner while frontmatter has not yet mirrored the choice.
    approved_with_concerns_active: Whether frontmatter records that the operator accepted the
      outstanding concerns as-is.
    continue_review_ticked: Whether the operator ticked "continue review cycle" on the
      concerns-decision banner.
    concerns_decision_pending: Whether the concerns-decision banner should be rendered this
      tick.
    any_unanswered_question: Whether the document contains an open question callout with no
      operator tick.
    section_layout: Configured slot ("top" or "bottom") for every registered section writer,
      keyed by expert name and section id.
    needs_bootstrap: Whether the file is newly opted in to review but still missing one or
      more reserved bootstrap fields.
    review_phase: Currently open post-approve barrier phase, or `None` when no barrier is open.
    barrier_writers_to_dispatch: Writers of the active barrier that need a fresh dispatch.
    barrier_open: Whether the active barrier still has a writer in flight.
    barrier_ready_to_collect: Whether every writer of the active barrier has finished and is
      ready to be collected in one pass.
    operator_reset_pending: Whether a post-approve operator edit outside every owned section
      invalidated the approval.
    historian_jobs_outstanding: Whether a historian job for this file is still in flight.
    waiting_context: Phase context used to enrich the "Waiting" banner title.
  """
...
class TickAction:
  """
  Action the dispatcher takes on a single tick, and its parameters.

  Attributes:
    kind: Action or phase identifier naming what the dispatcher should do this tick.
    expert: Expert reference the action applies to, when the action targets a specific expert.
    owned_section: Title of the H1 section the action applies to, when the action targets a
      specific section writer.
    banner_state: Banner state to render, when the action is a banner repaint.
    waiting_context: Phase context used to enrich the "Waiting" banner title, when applicable.
  """
...
def decide(inputs: TickInputs) -> TickAction:
  ...
  # a document the parser rejected goes to repair while attempts remain
  ...
  # an approved document still runs the post-approve validator barrier; it finalizes only once
  # that barrier settles, and an operator body-edit outside the owned sections resets the approval
  if inputs.approved:
  ...
  # a pending main writer is dispatched one at a time, in chain order
