...
**Operator-question invariant (MANDATORY for every operator question the writer emits, this mode and any other).** A question to the operator MUST carry answer options, or the dispatcher never sees the answer — ticking an option is the only answer signal a consumer can detect. When the writer wants free-form clarification instead, it writes the prompt as plain body prose, not as a question block. Whether a question also carries the escape row `- [ ] other — my answer in prose below` is the writer's judgement per question (`lazy-core.markdown-style.md` § Question callouts): add it when the listed options may not exhaust what the operator would say, leave it out when the answer set is closed by its nature.

**An answered question is a settled decision (MANDATORY).** A callout carrying a ticked option is the operator's decision: the writer folds it into the surrounding prose in the SAME round and removes the callout. This covers a ticked `[!decision-candidate]` exactly as it covers a ticked `[!question]`: on `accept` the candidate's thesis becomes prose (or a `[!decision]` statement when it passes the weight test), on `reject` the operator's prose written under the callout becomes the text, and the callout is gone either way. A ticked `other` row on a question is folded the same way as a rejected candidate: the operator's prose under the callout is the decision, the writer works it into the text, and the callout is gone. A candidate left in the body — ticked or not — is a questionnaire the finalize step refuses to ship. Re-asking the same question — in any rewording, with refined options, split into sub-cases — is a round defect, not diligence; a new question is legitimate only for a genuinely new gap the fold uncovered. When the ticked answer seems ambiguous or underspecified, the writer folds the literal choice as given and marks the residual call it had to make as a `[!decision-candidate]` — never by handing the operator the same fork twice.

**An open operator command is round input (MANDATORY).** A `[!todo] #review/command` callout in the source document whose mini-plan marks a writer step in progress is the operator's assignment for this round: the main writer carries the instruction out across its footprint. The callout itself is the dispatcher's pen — the writer returns it verbatim, never marks progress in it and never removes it.
