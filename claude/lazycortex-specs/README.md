...
| `lazy-spec.resolve-repo` | Dispatched by `lazy-spec.source-url` and by any `spec.*` skill that must turn a repo key from `lazy.settings.json[repos]` into a checkout path, branch, and forge — `lazy-spec.create-from-code`, `lazy-spec.sync-with-code`, `lazy-spec.doctor`, `lazy-spec.product-config`, `lazy-spec.rebase-pins`. Not for direct use: callers never inspect git remotes themselves, they call this once per repo per run and cache the record. |
