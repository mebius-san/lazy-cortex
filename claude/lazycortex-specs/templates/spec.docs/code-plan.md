---
tags:
  - {{product_tag}}
  - spec/empty
spec_role: code-plan
spec_doc_type: code-plan
wiki_pinned_topics:
  - wiki/doc-kind/code-plan
  - wiki/product/{{product}}
  - wiki/category/{{category}}
spec_stage: empty
spec_source_requests: []
spec_source_docs: []
---
# {{slug}} — code-plan

<!-- Development plan — populated once the feature's `design.md` is approved. Scope, sequence, and implementation notes for the developer. -->

## Goals
<!-- Every goal of the design, one line each, verbatim from it — a plan carries the whole design, and this list is what the final verification task and the test-plan are judged against. -->

## Tasks
<!-- Ordered tasks. Each names the exact files it creates, modifies, or deletes before its steps begin. -->

## Verification
<!-- The repository's own check and test commands, cited as they exist in the repo. The last task runs the whole check sweep and the entire test suite. -->

## Rollback
<!-- How to back the change out — a revert command for a small commit, a step-by-step sequence for a migration. -->

# Sources
#protected/spec/sources

## Requests
<!-- auto:spec-requests:start -->
<!-- auto:spec-requests:end -->

## Docs
<!-- auto:spec-docs:start -->
<!-- auto:spec-docs:end -->
