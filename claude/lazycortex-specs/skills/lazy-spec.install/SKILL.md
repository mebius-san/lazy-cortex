...
3. **Plugin-cache glob** (only when stages 1 and 2 found nothing). `Bash(ls -d ~/.claude/plugins/cache/*/lazycortex-wiki/*/bin/lazycortex-wiki 2>/dev/null)`. No output → `$WIKI_CLI` stays unresolved. One or more lines → each names a version-embedding path (`.../<version>/bin/lazycortex-wiki`); take the newest version — `Bash(ls -d ~/.claude/plugins/cache/*/lazycortex-wiki/*/bin/lazycortex-wiki 2>/dev/null | sort -V | tail -1)`, a numeric version sort so `10.0.0` outranks `9.1.1`, matching `axes.py`'s own newest-version pick — as `$WIKI_CLI`.
