...
# (opt-in sibling created by a ticked launch checkbox; seed-doc copies the status folder-note's
# spec_source_requests onto the seeded doc, so context_from_frontmatter folds the originating
# request(s) into the writer's bundle, same as design/bug)
- class: use-cases
  protocols: ["lazycortex-specs:lazy-spec.expert-signals-protocol"]
  paths:
