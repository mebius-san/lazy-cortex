...
`group_globs` collapses multiple changed paths under the same asset directory into ONE worker dispatch per tick (`lazy-core.routine-types-schema.md` § git `group_globs`) — two globs per registered product — `<vault_root>/<spec_path>/*/*` for an asset inside a folder and `<vault_root>/<spec_path>/*` for an asset straight at the product root; the daemon picks the deepest matching glob, so the list is a set and its order carries nothing (drop the `<vault_root>/` prefix when `spec.vault_root` is `.`, same as `path_filter`). A path at a glob's own depth — a group folder-note, or a product's own level document under the shallow glob — stays a single-file item, and a grouped member no status note owns — a nested product's own level document under the parent's shallow glob — is re-routed by `coordinator-dispatch` through the same single-file path, and any path outside every product's glob stays an ordinary single-file item — `coordinator-dispatch`'s `{path}` branch still serves those, resolving each to the folder-note that owns it; only paths strictly below the glob's depth collapse into the grouped `{dir, paths, ...}` item. The grouped `dir` is a coarse bucket, not an asset boundary: `coordinator-dispatch` splits the group's members by the nearest status folder-note at or below that dir and dispatches once per owner, so a nested asset's own documents wake the nested asset, not the enclosing one, and a nested product's assets (whose group dir carries no status note) still reach their own notes. **On a from-scratch install with zero products registered yet** (Step 7, the first product registration, runs after this one), core's `routine_types.py` rejects an empty `group_globs` list (`'group_globs' must be a non-empty list`) — OMIT the `group_globs` key from the `cfg` dict entirely in that case, rather than writing it as `[]`, as the JSON shape above shows for the ≥1-product case. `lazy-spec.product-config` Step 12 then CREATES the key, with this product's own two globs, the first time a product is registered or edited, and unions further products' globs into it from there. Re-running this step against an ALREADY-registered routine that lacks the key backfills it the same way — idempotently, as the union of every currently-registered product's two globs (empty when there are none, in which case the key stays omitted), never removing an operator-added entry.
...
2. **Dev-vault sibling stage** (only when stage 1 found nothing). In a checkout that carries the plugin sources themselves, the wiki's binary sits beside this plugin's own tree: `Bash(test -f claude/lazycortex-wiki/bin/lazycortex-wiki && echo claude/lazycortex-wiki/bin/lazycortex-wiki)`, run from the repo root. A printed path is `$WIKI_CLI` — skip stage 3. No output → go to stage 3. This stage is what keeps a dev checkout on its own sources instead of a stale installed copy, and it is the stage a plain interactive install most often lands on when the daemon never exported the env var.
