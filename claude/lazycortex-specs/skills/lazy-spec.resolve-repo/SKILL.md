...
description: "Dispatched by `lazy-spec.source-url` and by any `spec.*` skill that must turn a repo key from `lazy.settings.json[repos]` into a checkout path, branch, and forge — `lazy-spec.create-from-code`, `lazy-spec.sync-with-code`, `lazy-spec.doctor`, `lazy-spec.product-config`, `lazy-spec.finalize-branch`. Not for direct use: callers never inspect git remotes themselves, they call this once per repo per run and cache the record."
...
The authoritative definitions (the `repos` settings section shape, known-forges table) live in `${CLAUDE_PLUGIN_ROOT}/references/lazy-spec.config-protocol.md` and `${CLAUDE_PLUGIN_ROOT}/references/lazy-spec.sources-protocol.md`. This skill never restates those tables — it reads them.
...
If `<repo>` is not a key in the `repos` section, abort with a message telling the user to register the repo via `/lazy-spec.product-config` (the wizard that writes `lazy.settings.json[repos][<repo>]`).
...
- Else, look up `host` in the known-forges table in `${CLAUDE_PLUGIN_ROOT}/references/lazy-spec.sources-protocol.md` → "Known-forges table" (hostname-match column).
...
The `RepoInfo` record above. Callers pass the repo key and path to `lazy-spec.source-url` to get a forge-correct URL.

## Failure modes

- **`/lazy-spec.resolve-repo` aborts: repo key not registered** — `<key>` is not a key in `lazy.settings.json[repos]` → register the repo via `/lazy-spec.product-config`, then re-run.
- **`/lazy-spec.resolve-repo` aborts: missing `local_path` or `branch`** — the `repos[<key>]` record is incomplete → add `local_path:` and `branch:` to the record (optionally `forge:`) via `/lazy-spec.product-config` and re-run.
- **`/lazy-spec.resolve-repo` aborts: `local_path` is `.` but the current directory is not a git repo** — the record uses the same-repo `"."` form but `git rev-parse --show-toplevel` failed → run from inside the checkout that holds `.claude/lazy.settings.json`, or set an absolute `local_path`.
- **`/lazy-spec.resolve-repo` aborts: "no git remotes configured"** — `git remote` returned nothing for the checkout at `<local_path>` → add at least one remote (`git remote add origin <url>`) and re-run.
