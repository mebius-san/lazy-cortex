...
1. Run `"${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-specs" resolve-product by-key <key>` (or `by-path <path>` when given a path). It prints `{"key": "<product>", "record": <record-or-null>}`.
...
- **Container-note `#protected/spec/summary` section** — the product root folder-note and every container folder-note MUST each carry a `# Summary` section whose first content line is `#protected/spec/summary`. Missing `# Summary` section → FAIL. What that section must contain depends on which note it sits on, because three note kinds carry three shapes:
  ...
  - **Every other container folder-note** (`features/` / `changes/` / `bugs/` and any folder the operator adds) carries the stats marker alone. A category holds no description of its own, so a précis marker on one is a leftover from an older scaffold → WARN naming the note, with removal as the repair. Missing the stats marker → FAIL.
...
- [ ] `asset-type-missing`: `features/<feat>/<feat>.md` carries no `spec_asset_type` — run `"${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-specs" asset-type backfill` (`unknown` is a legal value, not a finding)
...
- [ ] `note-check` violation: `features/<feat>/<feat>.md` — `<violation kind>` on `<key-or-section>` (unknown-key / bad-type / missing-section / missing-marker / section-order, folded verbatim from `"${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-specs" note-check`)
...
- [ ] Leftover precis marker: `<spec_path>/features/features.md` `# Summary` body carries `<!-- spec:precis:* -->` (a category note holds no description of its own)
...
- [ ] Missing container `# Coordinator rules`: `<spec_path>/features/features.md` has no `# Coordinator rules` section
...
- [ ] Missing `doc-kind` pin: `features/<feat>/decisions.md` `wiki_pinned_topics` lacks `wiki/doc-kind/decisions` (spec-scope configured in `wiki.scopes`) — run `"${LAZYCORTEX_PYTHON:-python3}" "${CLAUDE_PLUGIN_ROOT}/bin/lazycortex-specs" pins`
