...
- **`spec_doc_type` present (FAIL)** — every authored catalog document carries `spec_doc_type`. Absence is a FAIL; the fix is `lazycortex-specs doc-type backfill`. Every folder-note is excluded — an asset's status note (`spec_role: status`), a level note of a product or of the catalog root (`spec_role: product` / `catalog`), and the operator-zone group and category notes alike. None of them has a type or may have one, so a `spec_doc_type` key found on any of them is itself a FAIL. Two verbs remove it and the finding names whichever fits: `lazycortex-specs doc-type backfill` strips it off every level note in the catalog in one walk, and `lazycortex-specs note-drop-key <note_dir> spec_doc_type` takes it off one note — the same verb the coordinators reach for on their own wake, so a level note under an active coordinator usually clears itself before this pass ever sees it.
