...
Then `AskUserQuestion` for the historian — single-select from the expert names, default `lazy-review.historian`. Stem: the historian writes the `# History` section of every reviewed doc across all built-in classes. Why-it-matters: it is the `experts.history` writer; it must be an expert registered with commit permission in the local repo.
