...
- **Default branch, blob**: `spec.source-url("backend", "myapp/chapter.py")` — most common; skills emitting source links in `tech` files.
- **Default branch, tree**: `spec.source-url("backend", "myapp/", kind="tree")` — directory references.
- **Pinned branch**: `spec.source-url("backend", "myapp/chapter.py", branch="feat/foo")` — when a `tech` or feature `tasks` file pins a branch via `source_branches: backend: feat/foo`.
