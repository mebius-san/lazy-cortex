...
Thin built-in wrapper that pins the asset type to `feature` and delegates to `lazy-spec.create-asset`. This skill asks no questions and writes no files itself — the universal `lazy-spec.create-asset` skill owns the wizard, scaffold, prose, and diagrams. See `${CLAUDE_PLUGIN_ROOT}/skills/lazy-spec.create-asset/`.
