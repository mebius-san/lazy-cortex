...
Capture a raw user idea into the vault-wide `requests/` inbox. The output is a body-only markdown file at `<vault-root>/requests/<slug>.md` — no frontmatter. The `lazy-spec.request-open` routine adds frontmatter on the next md-scan tick (per the new contract in `${CLAUDE_PLUGIN_ROOT}/references/lazy-spec.request-protocol.md` → "Lifecycle invariants" — the request-handling subsystem is the sole writer of frontmatter).
...
- **Body-only output.** This skill writes NO frontmatter. The request-handling subsystem (`lazy-spec.request-open` routine at open; `lazy-spec.request-apply` at apply) is the sole writer of `spec_role`, `request_status`, `request_class`, and the `request/<value>` mirror tag. (`spec.coordinator` runs during the review loop but writes only the body of its routing section, never frontmatter.) Adding frontmatter here would create a write-conflict.
...
- **Never classify.** This skill never sets `request_class`. The `lazy-spec.request-classify` primitive runs during the review loop (consumed in memory by `spec.coordinator`); the apply gate writes the field to frontmatter post-finalize, derived from the routing section the coordinator settled.
