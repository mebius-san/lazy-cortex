...
- Registering a product whose code lives in the very repo that holds the spec vault: run `/spec.product-config`, and when the source step asks for the repo's `local_path`, pick "this repo (.)" instead of typing an absolute path — the same product record then resolves correctly whether you are working in your dev checkout or a separate runtime clone.
...
- **Register a source repo that lives in the same checkout as the spec vault.** Run `/spec.product-config`, attach a new repo at the source step, and answer "this repo (.)" at the `local_path` question instead of typing a path — every checkout resolves its own root at read time, so the same product record works from your dev copy and from a separate runtime clone without editing settings per machine.
