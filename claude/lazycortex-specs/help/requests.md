...
**Override the spawn slug or any of the named fields.** The coordinator derives a slug from the request title. If you want it — or `path=`, `tools=`, `product=`, `targets=`, or `drop=` — different, edit the `routing-decision` block before confirming; the apply worker reads back whatever you left there.
