...
`/lazy-spec.product-config` Step 8 assigns the use-case-writer and ui-designer roles alongside the other eight, and generates their `use-cases` / `ui-design` review classes the same way it generates every other doc-kind class.
...
When a source document gets re-approved AFTER the dependent it feeds was already accepted, the coordinator compares their `# History` lines on the wake that sees it, finds the dependent now older than its source, moves the dependent back to `draft` (via `/lazy-spec.set-stage` — the one case where it lowers a stage on its own), flips the gate that dependent had closed back off, and immediately re-runs its upward checks in the same commit — so a `Review <file>` row, or a continuation tick on a tool's job, reappears the same cycle rather than lagging a tick behind. It writes one `# History` line and one `# Status brief` sentence naming what went stale, so the reason is visible without digging. A doc-transition wake whose value comes back empty just means the source re-entered review, not that it was re-approved — this rule fires only once the source itself lands `approved` again.
