...
**`/lazy-spec.sync-with-code`** is the commit-to-spec translator. You run it whenever source code has moved since the last sync. It reads the product's sync state to find the last-synced commit, collects every commit from that point to HEAD that touched the product's configured source paths, and fans large commit sets out to parallel read-only agents — one for structural changes (routes, classes, functions), one for data and template changes, one for user-visible behavior signals. The main session synthesizes those findings and presents a grouped summary before touching any file. User-visible behavior changes are surfaced as candidates for the product design doc that you approve or decline per item; code-level changes that alter nothing a user sees are recorded in the run log and written to no document at all. The product `tech.md` is never touched — it is a narrow hand-written document of stack, platforms, constraints, infrastructure decisions and boundaries, and it changes only through its own review. After approved prose rewrites land, the skill refreshes the diagram for any rewritten section that already carries one — it never adds a diagram to a section that had none — proposes `spec_develop_done` for any asset whose code objectively landed on the default branch, and proposes per-file stage corrections where the current stage is inconsistent with the code state. All gate flips and stage changes go through `/lazy-spec.flip-gate` and `/lazy-spec.set-stage` — the skill never edits gate frontmatter directly. Any document a run creates from scratch, rather than edits, is written parked at `spec_stage: deferred` — nothing automatic picks it up until you promote it with `/lazy-spec.set-stage <doc> draft`; a document the run only edits keeps whatever stage it already carried. Given an asset instead of a bare product key, it runs in asset mode instead: it reconciles that ONE feature/change asset's `design.md` / `architecture.md` against the current code by anchor — source links, domain groups, project structure — rather than by commit diff, and never rewrites design/architecture prose silently; every drift finding becomes an `[!attention]` callout, a change-asset proposal, or a gate-correction proposal.
...
**Newly authored docs start parked.** When a sync run creates a document rather than editing one, it lands at `spec_stage: deferred` — a fifth stage alongside `empty | draft | approved | rejected`. Nothing downstream (gates, the coordinator, another sync) reads a `deferred` doc until you move it to `draft` yourself with `/lazy-spec.set-stage <doc> draft`. This keeps a sync run from starting work on your behalf just because it noticed a gap.
...
The gates block provides the `/lazy-spec.flip-gate` and `/lazy-spec.set-stage` primitives that `sync-with-code` and `rebase-pins` call when they propose gate flips and stage corrections — code-sync surfaces the proposals; gates block skills execute the mutations.

The source-links block provides the repo resolution and forge-correct URL primitives that `sync-with-code` and `rebase-pins` rely on. Every source URL produced during sync or pin reconciliation goes through `/lazy-spec.source-url` — never an inline forge-specific path scheme.
...
flowchart TD
  ...
  runSyncWithCode[lazy-spec.sync-with-code: design-doc candidates, gate proposals, pin rewrites]
  runFinalizeBranch[lazy-spec.rebase-pins: spec_released proposal, pins finalized]
