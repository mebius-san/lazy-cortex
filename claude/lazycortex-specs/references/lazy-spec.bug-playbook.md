...
- **Documents.** Exactly one is mandatory — `bug.md`; `cancelled` is refused on it (the asset is abandoned as a whole through `spec_cancelled`, never through its primary document). Plans and reports belong to tools and are declared by `tool_types`; the `decisions` registry is opt-in and written only by the `lazy-spec.record-decision` primitive.
