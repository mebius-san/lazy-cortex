...
The `lazy-spec.request-open` routine populates the request half of the first-touch frontmatter — `spec_role`, `request_status: draft`, `request_class: unknown`, `tags: [request/draft]` — and delegates the review half to `lazycortex-review start --no-commit`, which writes all five `review_*` keys (`review_active`, `review_round`, `review_approved`, `review_phase`, `review_main_done`) plus the Waiting banner above the body. All five are required before the routine reads the file as opened rather than `partial`. The class stays `unknown` throughout the review loop; the apply worker overwrites it post-finalize when it stamps the terminal markers, reading the class verbatim from the verdict the router settled into its terminal section prose (see `${CLAUDE_PLUGIN_ROOT}/bin/apply_request.py`).
