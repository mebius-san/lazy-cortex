...
- **`vision`** (`vision.md`) — mandatory per the type record's `vision: mandatory` contract; the feature's goals live ONLY here (per `lazy-spec.doc-height-protocol.md`), the sections are the template's. Written by the designer. The coordinator seeds and starts it itself on the asset's first wake — the same seed-then-start flow as a checkbox row, without a checkbox — under one guard: only where `design.md` does not exist yet. A `design.md` already present without a sibling vision is a legal pre-vision state the coordinator never touches; such documents are migrated by the operator by hand.
- **`design`** (`design.md`) — always mandatory; the sections are the template's. Written by the designer. This document is never `cancelled` — the stage primitive refuses it, and cancelling an asset is the `spec_cancelled` overlay on the status folder-note, not a cancelled design.
- **`architecture`** (`architecture.md`) — mandatory from the moment the coordinator judges the asset code-bearing (below); the sections are the template's. Written by the architect. Never `cancelled` either.
...
- **`decisions`** — the asset's decision registry, kept by `lazy-spec.record-decision`. Created lazily by the first recorded decision; its absence means nothing and blocks nobody.
...
`design.md` launches through its own `Write design` checkbox like the rest — and on a feature that row does not appear until `vision.md` is `approved` (the vision is mandatory and seeded first, so the row's precondition reads its stage). Its hold is a precondition on continuing its review: when `use-cases.md` exists, the designer is not continued on `design.md`'s review until `use-cases.md` reaches `approved` or `cancelled`, and reads it once it is approved. The general rule is one sentence: a document's review proceeds only after every EXISTING upstream sibling in the reading chain (vision → use-cases → design → tech) is `approved`; an absent opt-in document never blocks. Either way the point is the same: the use cases are meant to settle before the behaviour they describe is written down for good, and the screens before the module boundaries built to serve them are cast.

**The windows.** A late vision edit follows the same boundary: past `spec_design_done`, a goals revision is a change asset's business. `Write use-cases` closes the moment `spec_design_done` closes — past that point a fresh use case is a change asset's business, not this asset's own definition half. `Write ui-design` closes on `architecture.md` reaching `approved` (or, for an asset that never becomes code-bearing, on `spec_plan_done` closing instead) — past that point a screen redesign likewise belongs to a change asset.
...
|---|---|---|
| `Write design` | asset exists AND `design.md` doesn't exist AND `vision.md` is `approved` | seed `design.md:design`, then the seed-then-start flow below |
