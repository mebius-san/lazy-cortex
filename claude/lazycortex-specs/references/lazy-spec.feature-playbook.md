...
- **`vision`** (`vision.md`) — mandatory per the type record's `vision: mandatory` contract. Goals, value, one-screen "what this is and for whom"; the feature's goals live ONLY here (per `lazy-spec.doc-height-protocol.md`). Written by the designer. The coordinator seeds and starts it itself on the asset's first wake — the same seed-then-start flow as a checkbox row, without a checkbox — under one guard: only where `design.md` does not exist yet. A `design.md` already present without a sibling vision is a legal pre-vision state the coordinator never touches; such documents are migrated by the operator by hand.
- **`design`** (`design.md`) — always mandatory. What the feature does and why: behaviour, not construction; its goals moved out to the sibling vision, which its opening references. Written by the designer. This document is never `cancelled` — the stage primitive refuses it, and cancelling an asset is the `spec_cancelled` overlay on the status folder-note, not a cancelled design.
