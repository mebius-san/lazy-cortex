...
Three kinds of folders exist under the vault root:
...
3. **Product folders** — leaf folders using lowercase-with-hyphens. A product folder owns the category subdirectories below — no loose files at the product folder level except the product folder-note (see "Folder-notes" below):
   - `docs/` — product-level `design.md` + `tech.md` plus the `docs/docs.md` operator folder-note. Required. There is no per-product `spec/` subfolder — the system itself is called "spec", so a nested `spec/` would be confusing; product-level reference material lives under `docs/`.
...
There is no per-asset `tech.md` (only the product carries `docs/tech.md`) and no `layout` doc.
...
```
.claude/templates/
├── spec.product/                                ← product-level docs (the docs/ subfolder)
│   ├── design.md                                ← docs/design.md
│   ├── tech.md                                  ← docs/tech.md
│   ├── group-note.md                            ← <product>.md operator folder-note (product is the "group" of its asset categories)
│   └── docs-note.md                             ← docs/docs.md operator folder-note for the docs/ sub-collection
...
└── [<Namespace>/]                           ← optional grouping folder, no role files
    └── <product>/                           ← product folder (lowercase-with-hyphens)
        ...
        ├── docs/                            ← product-level docs
        │   ├── docs.md                      ← operator folder-note (operator-zone; managed iconize_icon)
        │   ├── design.md
        │   └── tech.md
        ├── features/
...
| `tech` | Technical specification: architecture, source file map, components, data structures, reuse notes. Product-level only (`docs/tech.md`) — no per-asset `tech.md` | **Yes** | **Yes** | **Yes** |
...
| `status` | Asset folder-note: lifecycle state as flat gate booleans (`spec_design_done`…`spec_released` + `spec_cancelled`), `## Gates` callouts, `## History` log. See [lifecycle](./spec.lifecycle-protocol.md) | **No** | No | No — carries **gates**, not a per-file stage |
...
**Path constraints.** `status` files are only permitted at an asset folder-note path (`<spec_path>/<category>/<slug>/<slug>.md`) — never at the product root, never under `docs/`. `bug` files are only permitted under `<spec_path>/bugs/<slug>/`. `design.md` / `plan.md` live in an asset folder; `docs/design.md` + `docs/tech.md` are the product-level docs.
...
|------|----------|---------------|
| `design` | `design.md` | `docs/`, `<spec_path>/<category>/<slug>/` (any category except `bugs`) |
| `tech` | `tech.md` | `docs/` (product-level only) |
...
- `[[Server/Tester/chapter/docs/design|chapter design]]`
...
After frontmatter and the H1 title, the folder-note carries exactly two H2 sections in this order: `## Gates` and `## History`. There is no `## Current`, no `## Workflow`, no `## Log`.
...
| Product-level `docs/design.md` | `../../docs/design.md` |
| Product-level `docs/tech.md` | `../../docs/tech.md` |
