...
| `research` | The research report — the `research` tool's deliverable on a research asset, sections per its template. The only authored doc where external URLs are allowed — in `## Sources` and beside the finding that cites them. Its questions live in the sibling `design.md` typed `research-design` | **Yes** (only in `## Sources` and inline beside a finding) | No | **Yes** |
...
| `spec_stage` | stage-bearing authored docs (`vision`, `system-vision`, `use-cases`, `design`, `system-design`, `architecture`, `ui-design`, `system-ui-design`, `code-plan`, `test-plan`, `bug`, `system-tech`) | per-file lifecycle stage, one of `empty | draft | approved | rejected | cancelled | deferred`; mirrored to a `spec/<stage>` tag. See [lifecycle](./lazy-spec.lifecycle-protocol.md). `code-report` / `test-report` / `decisions` carry NO `spec_stage` |
