---
role: plugin-status
stage: published
plugin: lazycortex-specs
current_version: 7.4.0
published_version: 7.4.0
generated_at: 2026-09-07 19:30:12 UTC
iconize_icon: LiFileClock
iconize_color: "#86efac"
---
# lazycortex-specs — up to date

**Status**: published — version `7.4.0` matches the top of `CHANGELOG.public.md` and no commits have landed under `claude/lazycortex-specs/` since.

---
*Frontmatter + status header generated by the publish-status agent. The `## Highlights since …` body is curated by the pre-commit pipeline on each meaningful commit; treat it as LLM-maintained narrative.*
