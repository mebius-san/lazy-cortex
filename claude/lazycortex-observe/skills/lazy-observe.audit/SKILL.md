...
- `FAIL endpoint-down (<repo_label> ...)` — that repo's lazycortex-core daemon is down, or its `metrics.enabled` is `false` → see `claude/lazycortex-core/references/lazy-core.metrics-schema.md` for the settings the daemon reads.
...
- `FAIL no-daemons-registered` — no repo on this host has a metrics-enabled daemon → see `claude/lazycortex-core/references/lazy-core.metrics-schema.md` for the settings that register one.
