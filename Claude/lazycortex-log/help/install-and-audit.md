...
`/lazy-log.install` detects whether the plugin is enabled at project or user scope, then works through nine ordered steps to set up everything the plugin needs.
...
Beyond rule files, the skill creates `.logs/changelog.md` if it does not already exist (the seed file that `lazy-log.distill` will later populate), appends a `.logs/` entry to `.gitignore` so that per-contributor run logs and the structured commit log stay out of version control, and seeds `lazy.settings.json` with `agent_models` entries for every log agent the plugin ships. Tier values for those entries come from `lazycortex-core`'s `default-tiers.json` at runtime — the skill never hardcodes them. It then registers the `lazy-log.commit-recorder` post-commit hook so that every subsequent `git commit` lands in `.logs/commits.jsonl` automatically.

The final install step scans your project's skills, agents, and commands for waiver candidates — artifacts that are exempt from the standard run-log requirement (for example, synthesizer agents that produce their output in-session rather than writing files). For each candidate the skill asks whether to add a `logging-waiver:` field to the artifact's frontmatter, and offers a pre-written suggested reason so the decision is low-friction. You can skip the scan entirely by passing `--no-scan`. The whole sequence is idempotent: re-running after a plugin update or a manual rule edit is always safe.
...
The inline checks confirm that `.claude/rules/lazy-log.logging.md` is present, has a `description` field in its frontmatter, specifies `./.logs/claude/<name>/` as the log path, references `git_sha` capture via `git rev-parse HEAD`, uses UTC timestamps, and calls out the two-step write pattern (`Bash(mkdir -p ...)` then the `Write` tool — never chained with `&&`). It also validates every `logging-waiver:` field it encounters: a waiver whose value is blank, the literal `true`, or the literal `yes` is flagged `[FAIL]` — those forms are syntactically present but convey no meaningful reason, which defeats the purpose of the waiver.

Any per-file `## Logging` sections that point to a different path, use a non-standard timestamp format, or suggest chaining are flagged as `[WARN]`. A missing rule file is `[FAIL]` and short-circuits the cross-checks. After collecting all findings, the skill presents a structured report — summary counts, per-finding detail, and a waiver-candidates section listing any artifacts that appear to need a waiver but don't have one yet. Nothing is mutated without confirmation.

## How they work together

Install bootstraps; audit verifies later. The typical sequence is: enable the plugin, run `/lazy-log.install` once to wire everything up, then let the project proceed. Audit enters the picture in four situations:
...
- When waiver coverage drifts — if you add a new synthesizer agent or ephemeral-style command, `/lazy-log.audit` flags it as a waiver candidate and offers to add the `logging-waiver:` field interactively. The same audit also catches existing waivers whose value was set to a bare `true` or left empty, which install's `--no-scan` path can produce if skipped carelessly.
...
- The **housekeeping block** (`/lazy-log.clean`) can classify orphaned log folders accurately because the canonical skill namespace is stable, the logging rule is coherent, and waiver-exempt agents are correctly marked so their absent log dirs are not misclassified as orphans.
