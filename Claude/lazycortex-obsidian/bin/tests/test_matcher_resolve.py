...
ICON_MAP = json.loads((pathlib.Path(__file__).parent / "fixtures" / "obsidian-icon-map.json").read_text())
...
def test_resolve_status_file_emits_self_only_in_schema_2():
    """Schema 2 always emits a single self-keyed entry; the legacy schema-1
    `parent_dir` emit was dropped when the worker became frontmatter-only."""
    entries = isync.resolve_matchers(ICON_MAP, "app/feature/_folder.md",
                                     {"stage": "draft-design"})
    assert entries == [("app/feature/_folder.md",
                        {"iconName": "LiPencil", "iconColor": "#fde68a"})]
