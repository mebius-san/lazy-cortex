...
- **Stage → commit promptly.** Once you've started staging, finish the commit. Leaving a non-empty index sitting for ten minutes triggers the stale-and-idle break-rule — your work isn't lost (it stays in the working tree), but another session may break the lock.
